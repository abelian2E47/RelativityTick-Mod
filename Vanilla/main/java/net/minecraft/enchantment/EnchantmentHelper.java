...
public class EnchantmentHelper {
    ...
    private static void forEachEnchantment(
        ItemStack stack, EquipmentSlot slot, LivingEntity entity, EnchantmentHelper.ContextAwareConsumer contextAwareConsumer
    ) {
        if (!stack.isEmpty()) {
        ...
        if (target instanceof LivingEntity livingEntity) {
            forEachEnchantment(
                ...
                (enchantment, level, context) -> enchantment.value()
                    .onTargetDamaged(world, level, context, EnchantmentEffectTarget.VICTIM, target, damageSource)
            );
        }

        if (weapon != null) {
            if (damageSource.getAttacker() instanceof LivingEntity livingEntity2) {
                forEachEnchantment(
                    weapon,
