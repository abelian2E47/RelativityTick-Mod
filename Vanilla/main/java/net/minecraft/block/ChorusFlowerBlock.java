...
public class ChorusFlowerBlock extends Block {
        ...
        if (world.isAir(blockPos) && blockPos.getY() <= world.getTopYInclusive()) {
            ...
            if (i < 5) {
                ...
                if (bl && isSurroundedByAir(world, blockPos, null) && world.isAir(pos.up(2))) {
                    ...
                    if (bl3) {
                        world.setBlockState(
                            pos, ChorusPlantBlock.withConnectionProperties(world, pos, this.plantBlock.getDefaultState()), Block.NOTIFY_LISTENERS
                        );
                    } else {
                        this.die(world, pos);
                    }
    ...
    private static void generate(WorldAccess world, BlockPos pos, Random random, BlockPos rootPos, int size, int layer) {
        ...
        for (int j = 0; j < i; j++) {
            ...
            world.setBlockState(
                blockPos.down(), ChorusPlantBlock.withConnectionProperties(world, blockPos.down(), block.getDefaultState()), Block.NOTIFY_LISTENERS
            );
        }
