...
public class PistonBlock extends FacingBlock {
                ...
                if (!bl2) {
                    if (type != 1
                        ...
                        || blockState3.getPistonBehavior() != PistonBehavior.NORMAL
                            && !blockState3.isOf(Blocks.PISTON)
                            && !blockState3.isOf(Blocks.STICKY_PISTON)) {
    ...
    private boolean move(World world, BlockPos pos, Direction dir, boolean extend) {
        ...
        for (int k = list.size() - 1; k >= 0; k--) {
            ...
            world.addBlockEntity(PistonExtensionBlock.createBlockEntityPiston(blockPos4, blockState4, list2.get(k), dir, extend, false));
        ...
        if (extend) {
            ...
            BlockState blockState6 = Blocks.MOVING_PISTON
                ...
                .with(PistonExtensionBlock.TYPE, this.sticky ? PistonType.STICKY : PistonType.DEFAULT);
            map.remove(blockPos);
            world.setBlockState(blockPos, blockState6, Block.NO_REDRAW | Block.MOVED);
        ...
        for (Entry<BlockPos, BlockState> entry : map.entrySet()) {
            ...
            blockState7.updateNeighbors(world, blockPos6, Block.NOTIFY_LISTENERS);
        ...
        for (int l = list3.size() - 1; l >= 0; l--) {
            ...
            world.updateNeighborsAlways(blockPos7, blockState9.getBlock(), wireOrientation);
        }

        for (int m = list.size() - 1; m >= 0; m--) {
            world.updateNeighborsAlways(list.get(m), blockStates[i++].getBlock(), wireOrientation);
        }
