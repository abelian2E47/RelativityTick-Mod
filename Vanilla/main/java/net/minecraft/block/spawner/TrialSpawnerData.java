    ...
    public void updatePlayers(ServerWorld world, BlockPos pos, TrialSpawnerLogic logic) {
        ...
        if (!bl) {
            if (!logic.getSpawnerState().equals(TrialSpawnerState.COOLDOWN) || !logic.isOminous()) {
                ...
                if (!logic.getSpawnerState().equals(TrialSpawnerState.COOLDOWN) || bl3) {
                    boolean bl4 = logic.getData().players.isEmpty();
                    List<UUID> list2 = bl4 ? list : logic.getEntityDetector().detect(world, logic.getEntitySelector(), pos, logic.getDetectionRadius(), false);
        ...
        this.spawnData
            .ifPresent(
                spawnData -> nbtCompound.put(
                    "spawn_data",
                    MobSpawnerEntry.CODEC.encodeStart(NbtOps.INSTANCE, spawnData).result().orElseThrow(() -> new IllegalStateException("Invalid SpawnData"))
                )
            );
        return nbtCompound;
    }
