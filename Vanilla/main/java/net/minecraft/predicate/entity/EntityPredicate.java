...
public record EntityPredicate(
    ...
    public boolean test(ServerWorld world, @Nullable Vec3d pos, @Nullable Entity entity) {
        ...
        if (this.location.steppingOn.isPresent()) {
            Vec3d vec3d3 = Vec3d.ofCenter(entity.getSteppingPos());
            if (!this.location.steppingOn.get().test(world, vec3d3.getX(), vec3d3.getY(), vec3d3.getZ())) {
                return false;
            }
        }

        if (this.location.affectsMovement.isPresent()) {
            Vec3d vec3d4 = Vec3d.ofCenter(entity.getVelocityAffectingPos());
            if (!this.location.affectsMovement.get().test(world, vec3d4.getX(), vec3d4.getY(), vec3d4.getZ())) {
                return false;
            }
        }
