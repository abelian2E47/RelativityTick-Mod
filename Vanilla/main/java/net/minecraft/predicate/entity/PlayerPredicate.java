    ...
    public boolean test(Entity entity, ServerWorld world, @Nullable Vec3d pos) {
        if (!(entity instanceof ServerPlayerEntity serverPlayerEntity)) {
            ...
            for (Entry<RegistryKey<Recipe<?>>> entry : this.recipes.object2BooleanEntrySet()) {
                if (serverRecipeBook.isUnlocked(entry.getKey()) != entry.getBooleanValue()) {
                    return false;
                }
            }
    ...
    record StatMatcher<T>(StatType<T> type, RegistryEntry<T> value, NumberRange.IntRange range, Supplier<Stat<T>> stat) {
        ...
        private static <T> MapCodec<PlayerPredicate.StatMatcher<T>> createCodec(StatType<T> type) {
            return RecordCodecBuilder.mapCodec(
                instance -> instance.group(
                        (App<Mu<? extends PlayerPredicate.StatMatcher<?>>, RegistryEntry<T>>)type.getRegistry()
                            .getEntryCodec()
                            .fieldOf("stat")
                            .forGetter(PlayerPredicate.StatMatcher::value),
                        (App<Mu<? extends PlayerPredicate.StatMatcher<?>>, NumberRange.IntRange>)NumberRange.IntRange.CODEC
