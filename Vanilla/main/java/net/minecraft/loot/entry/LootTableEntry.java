...
public class LootTableEntry extends LeafEntry {
    ...
    public void generateLoot(Consumer<ItemStack> lootConsumer, LootContext context) {
        this.value
            .map(
                key -> context.getLookup().getOptionalEntry((RegistryKey<LootTable>)key).map(RegistryEntry::value).orElse(LootTable.EMPTY),
        ...
        this.value
            .ifLeft(
                key -> reporter.getDataLookup()
                    .getOptionalEntry((RegistryKey<LootTable>)key)
                    .ifPresentOrElse(
                        entry -> entry.value().validate(reporter.makeChild("->{" + key.getValue() + "}", (RegistryKey<?>)key)),
