...
public class QuickPlay {
    ...
    private static void startRealms(MinecraftClient client, RealmsClient realmsClient, String realmId) {
        ...
        try {
            ...
            Screen screen2 = new TitleScreen();
            client.setScreen(new DisconnectedScreen(screen2, ERROR_TITLE, ERROR_REALM_CONNECT, TO_TITLE));
            return;
        }

        RealmsServer realmsServer = realmsServerList.servers.stream().filter(server -> server.id == l).findFirst().orElse(null);
        if (realmsServer == null) {
            Screen screen3 = new RealmsMainScreen(new TitleScreen());
            client.setScreen(new DisconnectedScreen(screen3, ERROR_TITLE, ERROR_REALM_PERMISSION, TO_REALMS));
        } else {
