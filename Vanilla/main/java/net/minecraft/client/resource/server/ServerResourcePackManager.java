...
public class ServerResourcePackManager {
    ...
    private void onDownload(Collection<ServerResourcePackManager.PackEntry> packs, Downloader.DownloadResult result) {
        ...
        for (ServerResourcePackManager.PackEntry packEntry2 : packs) {
            Path path = result.downloaded().get(packEntry2.id);
            if (path != null) {
                packEntry2.loadStatus = ServerResourcePackManager.LoadStatus.DONE;
    ...
    private void applyDownloadedPacks() {
        ...
        if (bl) {
            for (ServerResourcePackManager.PackEntry packEntry2 : list) {
                if (packEntry2.status != ServerResourcePackManager.Status.ACTIVE) {
                    packEntry2.status = ServerResourcePackManager.Status.PENDING;
                }
            }

            for (ServerResourcePackManager.PackEntry packEntry3 : list2) {
                packEntry3.status = ServerResourcePackManager.Status.PENDING;
            }
            ...
            this.reloadScheduler.scheduleReload(new ReloadScheduler.ReloadContext() {
                ...
                public void onSuccess() {
                    ...
                    for (ServerResourcePackManager.PackEntry packEntry2 : list2) {
                        packEntry2.status = ServerResourcePackManager.Status.INACTIVE;
                    }
                ...
                public void onFailure(boolean force) {
                    if (!force) {
                        ...
                        for (ServerResourcePackManager.PackEntry packEntry2 : ServerResourcePackManager.this.packs) {
                            if (packEntry2.status == ServerResourcePackManager.Status.PENDING) {
                                packEntry2.status = ServerResourcePackManager.Status.INACTIVE;
                            }
                        }
                    }
