...
public class Mouse {
                ...
                if (this.client.currentScreen == null) {
                    ...
                    if (bl) {
                        ...
                        try {
                            ...
                            CrashReport crashReport2 = CrashReport.create(throwable2, "mouseReleased event handler");
                            screen.addCrashReportSection(crashReport2);
                            CrashReportSection crashReportSection2 = crashReport2.addElement("Mouse");
                            crashReportSection2.add("Scaled X", d);
                            crashReportSection2.add("Scaled Y", e);
                            crashReportSection2.add("Button", i);
    ...
    public void setup(long window) {
        InputUtil.setMouseCallbacks(
            ...
            (windowx, count, names) -> {
                ...
                if (!list.isEmpty()) {
                    int k = i;
                    this.client.execute(() -> this.onFilesDropped(windowx, list, k));
                }
            }
        );
    }
    ...
    private void updateMouse(double timeDelta) {
        ...
        if (this.client.player != null) {
            this.client.player.changeLookDirection(i, j * o);
        }
    }
