...
public class BeeEntityModel extends EntityModel<BeeEntityRenderState> {
    ...
    public void setAngles(BeeEntityRenderState beeEntityRenderState) {
        ...
        if (!beeEntityRenderState.angry && !beeEntityRenderState.stoppedOnGround) {
            float g = MathHelper.cos(beeEntityRenderState.age * 0.18F);
            this.bone.pitch = 0.1F + g * (float) Math.PI * 0.025F;
            this.leftAntenna.pitch = g * (float) Math.PI * 0.03F;
            this.rightAntenna.pitch = g * (float) Math.PI * 0.03F;
            this.frontLegs.pitch = -g * (float) Math.PI * 0.1F + (float) (Math.PI / 8);
            this.backLegs.pitch = -g * (float) Math.PI * 0.05F + (float) (Math.PI / 4);
