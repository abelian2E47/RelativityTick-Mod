...
public class RavagerEntityModel extends EntityModel<RavagerEntityRenderState> {
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData modelPartData2 = modelPartData.addChild(
            ...
            ModelTransform.pivot(0.0F, -7.0F, 5.5F)
        );
    ...
    public void setAngles(RavagerEntityRenderState ravagerEntityRenderState) {
        ...
        if (g > 0.0F) {
            ...
            float m = -1.0F;
            float n = -1.0F * MathHelper.sin(this.neck.pitch);
            this.neck.pivotX = 0.0F;
            this.neck.pivotY = -7.0F - n;
            ...
            if (bl) {
                ...
                float o = MathHelper.sin(ravagerEntityRenderState.roarTick * (float) Math.PI * 0.25F);
                this.jaw.pitch = (float) (Math.PI / 2) * o;
            }
        }
        ...
        float p = ravagerEntityRenderState.limbFrequency;
        float q = 0.4F * ravagerEntityRenderState.limbAmplitudeMultiplier;
        this.rightHindLeg.pitch = MathHelper.cos(p * 0.6662F) * q;
        this.leftHindLeg.pitch = MathHelper.cos(p * 0.6662F + (float) Math.PI) * q;
        this.rightFrontLeg.pitch = MathHelper.cos(p * 0.6662F + (float) Math.PI) * q;
        this.leftFrontLeg.pitch = MathHelper.cos(p * 0.6662F) * q;
    }
}
