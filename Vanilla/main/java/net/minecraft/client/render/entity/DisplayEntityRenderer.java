...
public abstract class DisplayEntityRenderer<T extends DisplayEntity, S, ST extends DisplayEntityRenderState> extends EntityRenderer<T, ST> {
    ...
    private Quaternionf getBillboardRotation(DisplayEntity.RenderState renderState, ST state, Quaternionf quaternionf) {
        ...
        return switch (renderState.billboardConstraints()) {
            ...
            case CENTER -> quaternionf.rotationYXZ(
                (float) (-Math.PI / 180.0) * getBackwardsYaw(camera), (float) (Math.PI / 180.0) * getNegatedPitch(camera), 0.0F
            );
        };
    }
    ...
    public static class TextDisplayEntityRenderer
        ...
        public void render(
            ...
            int l = 1;
            int m = 9 + 1;
            int n = textLines.width();
            int o = textLines.lines().size() * m - 1;
            matrix4f.translate(1.0F - n / 2.0F, -o, 0.0F);
            if (j != 0) {
                VertexConsumer vertexConsumer = vertexConsumerProvider.getBuffer(
                    bl ? RenderLayer.getTextBackgroundSeeThrough() : RenderLayer.getTextBackground()
                );
                vertexConsumer.vertex(matrix4f, -1.0F, -1.0F, 0.0F).color(j).light(i);
                vertexConsumer.vertex(matrix4f, -1.0F, o, 0.0F).color(j).light(i);
                vertexConsumer.vertex(matrix4f, n, o, 0.0F).color(j).light(i);
                vertexConsumer.vertex(matrix4f, n, -1.0F, 0.0F).color(j).light(i);
            }

            for (DisplayEntity.TextDisplayEntity.TextLine textLine : textLines.lines()) {
                float p = switch (textAlignment) {
                    case LEFT -> 0.0F;
                    case RIGHT -> n - textLine.width();
                    case CENTER -> n / 2.0F - textLine.width() / 2.0F;
                };
