...
public class BackgroundRenderer {
    ...
    public static Vector4f getFogColor(Camera camera, float tickDelta, ClientWorld world, int clampedViewDistance, float skyDarkness) {
        ...
        if (cameraSubmersionType == CameraSubmersionType.WATER) {
            ...
            float ab = 0.25F + 0.75F * clampedViewDistance / 32.0F;
            ab = 1.0F - (float)Math.pow(ab, 0.25);
            int bb = world.getSkyColor(camera.getPos(), tickDelta);
            float cb = ColorHelper.getRedFloat(bb);
            float db = ColorHelper.getGreenFloat(bb);
            float eb = ColorHelper.getBlueFloat(bb);
            float fb = MathHelper.clamp(MathHelper.cos(world.getSkyAngle(tickDelta) * (float) (Math.PI * 2)) * 2.0F + 0.5F, 0.0F, 1.0F);
            BiomeAccess biomeAccess = world.getBiomeAccess();
            Vec3d vec3d = camera.getPos().subtract(2.0, 2.0, 2.0).multiply(0.25);
            Vec3d vec3d2 = CubicSampler.sampleColor(
                vec3d,
                (i, j, k) -> world.getDimensionEffects().adjustFogColor(Vec3d.unpackRgb(biomeAccess.getBiomeForNoiseGen(i, j, k).value().getFogColor()), fb)
            );
            r = (float)vec3d2.getX();
            s = (float)vec3d2.getY();
            t = (float)vec3d2.getZ();
            if (clampedViewDistance >= 4) {
                float jb = MathHelper.sin(world.getSkyAngleRadians(tickDelta)) > 0.0F ? -1.0F : 1.0F;
                Vector3f vector3f = new Vector3f(jb, 0.0F, 0.0F);
                float kb = camera.getHorizontalPlane().dot(vector3f);
                if (kb < 0.0F) {
                    kb = 0.0F;
                }

                if (kb > 0.0F && world.getDimensionEffects().isSunRisingOrSetting(world.getSkyAngle(tickDelta))) {
                    int lb = world.getDimensionEffects().getSkyColor(world.getSkyAngle(tickDelta));
                    kb *= ColorHelper.getAlphaFloat(lb);
                    r = r * (1.0F - kb) + ColorHelper.getRedFloat(lb) * kb;
                    s = s * (1.0F - kb) + ColorHelper.getGreenFloat(lb) * kb;
                    t = t * (1.0F - kb) + ColorHelper.getBlueFloat(lb) * kb;
                }
            }
            ...
            s += (db - s) * ab;
            t += (eb - t) * ab;
            float mb = world.getRainGradient(tickDelta);
            if (mb > 0.0F) {
                float nb = 1.0F - mb * 0.5F;
                float ob = 1.0F - mb * 0.4F;
                r *= nb;
                s *= nb;
                t *= ob;
            }

            float pb = world.getThunderGradient(tickDelta);
            if (pb > 0.0F) {
                float qb = 1.0F - pb * 0.5F;
                r *= qb;
                s *= qb;
                t *= qb;
            }
        ...
        float rb = ((float)camera.getPos().y - world.getBottomY()) * world.getLevelProperties().getHorizonShadingRatio();
        ...
        if (statusEffectFogModifier != null) {
            ...
            rb = statusEffectFogModifier.applyColorModifier(
                livingEntity, livingEntity.getStatusEffect(statusEffectFogModifier.getStatusEffect()), rb, tickDelta
            );
        }

        if (rb < 1.0F && cameraSubmersionType != CameraSubmersionType.LAVA && cameraSubmersionType != CameraSubmersionType.POWDER_SNOW) {
            if (rb < 0.0F) {
                rb = 0.0F;
            }
        ...
        if (cameraSubmersionType == CameraSubmersionType.WATER) {
            if (entity instanceof ClientPlayerEntity) {
                sb = ((ClientPlayerEntity)entity).getUnderwaterVisibility();
            } else {
                sb = 1.0F;
            }
            ...
            sb = GameRenderer.getNightVisionStrength(livingEntity2, tickDelta);
        } else {
            sb = 0.0F;
        }

        if (r != 0.0F && s != 0.0F && t != 0.0F) {
            float wb = Math.min(1.0F / r, Math.min(1.0F / s, 1.0F / t));
            r = r * (1.0F - sb) + r * wb * sb;
            s = s * (1.0F - sb) + s * wb * sb;
            t = t * (1.0F - sb) + t * wb * sb;
        }
