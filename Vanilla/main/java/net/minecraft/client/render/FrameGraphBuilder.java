...
public class FrameGraphBuilder {
    ...
    public void run(ObjectAllocator allocator, FrameGraphBuilder.Profiler profiler) {
        ...
        for (FrameGraphBuilder.FramePass framePass2 : list) {
            for (FrameGraphBuilder.ResourceNode<?> resourceNode : framePass2.resourcesToAcquire) {
                profiler.acquire(resourceNode.name);
                resourceNode.acquire(allocator);
            }

            profiler.push(framePass2.name);
            framePass2.renderer.run();
            profiler.pop(framePass2.name);

            for (int i = framePass2.resourcesToRelease.nextSetBit(0); i >= 0; i = framePass2.resourcesToRelease.nextSetBit(i + 1)) {
                FrameGraphBuilder.ResourceNode<?> resourceNode2 = this.resourceNodes.get(i);
                profiler.release(resourceNode2.name);
                resourceNode2.release(allocator);
            }
        }
    }
