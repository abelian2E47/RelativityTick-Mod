...
public class WorldRenderer implements SynchronousResourceReloader, AutoCloseable {
    ...
    public void loadEntityOutlinePostProcessor() {
        ...
        this.entityOutlineFramebuffer = new SimpleFramebuffer(
            this.client.getWindow().getFramebufferWidth(), this.client.getWindow().getFramebufferHeight(), true
        );
    ...
    private void setupTerrain(Camera camera, Frustum frustum, boolean hasForcedFrustum, boolean spectator) {
        ...
        if (!hasForcedFrustum) {
            ...
            this.chunkRenderingDataPreparer
                .updateSectionOcclusionGraph(bl, camera, frustum, this.builtChunks, this.world.getChunkManager().getActiveSections());
            profiler.pop();
    ...
    private void renderBlockDamage(MatrixStack matrices, Camera camera, VertexConsumerProvider.Immediate vertexConsumers) {
        ...
        for (Entry<SortedSet<BlockBreakingInfo>> entry : this.blockBreakingProgressions.long2ObjectEntrySet()) {
            ...
            if (!(blockPos.getSquaredDistanceFromCenter(d, e, f) > 1024.0)) {
                SortedSet<BlockBreakingInfo> sortedSet = entry.getValue();
                if (sortedSet != null && !sortedSet.isEmpty()) {
                    int i = sortedSet.last().getStage();
                    matrices.push();
                    matrices.translate(blockPos.getX() - d, blockPos.getY() - e, blockPos.getZ() - f);
        ...
        if (this.client.crosshairTarget instanceof BlockHitResult blockHitResult) {
            if (blockHitResult.getType() != HitResult.Type.MISS) {
                ...
                if (!blockState.isAir() && this.world.getWorldBorder().contains(blockPos)) {
                    ...
                    this.drawBlockOutline(matrices, vertexConsumer2, camera.getFocusedEntity(), vec3d.x, vec3d.y, vec3d.z, blockPos, blockState, i);
    ...
    private void renderEntity(
        ...
        this.entityRenderDispatcher
            .render(
                entity, d - cameraX, e - cameraY, f - cameraZ, tickDelta, matrices, vertexConsumers, this.entityRenderDispatcher.getLight(entity, tickDelta)
            );
    }
            ...
            for (int j = pos.getX() - 1; j <= pos.getX() + 1; j++) {
                for (int k = pos.getY() - 1; k <= pos.getY() + 1; k++) {
                    this.scheduleChunkRender(
                        ChunkSectionPos.getSectionCoord(j), ChunkSectionPos.getSectionCoord(k), ChunkSectionPos.getSectionCoord(i), important
                    );
                }
            }
        }
    ...
    private Particle spawnParticle(
        ...
        if (force) {
            ...
            return particlesMode == ParticlesMode.MINIMAL
                ? null
                : this.client.particleManager.addParticle(parameters, x, y, z, velocityX, velocityY, velocityZ);
        }
    }
    ...
    public void setBlockBreakingInfo(int entityId, BlockPos pos, int stage) {
        if (stage >= 0 && stage < 10) {
            BlockBreakingInfo blockBreakingInfo2 = this.blockBreakingInfos.get(entityId);
            ...
            this.blockBreakingProgressions.computeIfAbsent(blockBreakingInfo2.getPos().asLong(), l -> Sets.newTreeSet()).add(blockBreakingInfo2);
        } else {
