...
public class BlockModelRenderer {
    ...
    static class AmbientOcclusionCalculator {
        ...
        public void apply(BlockRenderView world, BlockState state, BlockPos pos, Direction direction, float[] fs, BitSet bitSet, boolean bl) {
            ...
            if (!bl5 && !bl3) {
                ...
                bb = brightnessCache.getFloat(blockState12, world, mutable);
                cb = brightnessCache.getInt(blockState12, world, mutable);
            }

            int db = brightnessCache.getInt(state, world, pos);
            mutable.set(pos, direction);
            BlockState blockState13 = world.getBlockState(mutable);
            if (bitSet.get(0) || !blockState13.isOpaqueFullCube()) {
                db = brightnessCache.getInt(blockState13, world, mutable);
            }

            float eb = bitSet.get(0)
            ...
            if (bitSet.get(1) && neighborData.nonCubicWeight) {
                float jb = (m + f + t + eb) * 0.25F;
                float kb = (h + f + p + eb) * 0.25F;
                float lb = (h + g + x + eb) * 0.25F;
                float mb = (m + g + bb + eb) * 0.25F;
                float nb = fs[neighborData.field_4192[0].shape] * fs[neighborData.field_4192[1].shape];
                float ob = fs[neighborData.field_4192[2].shape] * fs[neighborData.field_4192[3].shape];
                float pb = fs[neighborData.field_4192[4].shape] * fs[neighborData.field_4192[5].shape];
                float qb = fs[neighborData.field_4192[6].shape] * fs[neighborData.field_4192[7].shape];
                float rb = fs[neighborData.field_4185[0].shape] * fs[neighborData.field_4185[1].shape];
                float sb = fs[neighborData.field_4185[2].shape] * fs[neighborData.field_4185[3].shape];
                float tb = fs[neighborData.field_4185[4].shape] * fs[neighborData.field_4185[5].shape];
                float ub = fs[neighborData.field_4185[6].shape] * fs[neighborData.field_4185[7].shape];
                float vb = fs[neighborData.field_4180[0].shape] * fs[neighborData.field_4180[1].shape];
                float wb = fs[neighborData.field_4180[2].shape] * fs[neighborData.field_4180[3].shape];
                float xb = fs[neighborData.field_4180[4].shape] * fs[neighborData.field_4180[5].shape];
                float yb = fs[neighborData.field_4180[6].shape] * fs[neighborData.field_4180[7].shape];
                float zb = fs[neighborData.field_4188[0].shape] * fs[neighborData.field_4188[1].shape];
                float ac = fs[neighborData.field_4188[2].shape] * fs[neighborData.field_4188[3].shape];
                float bc = fs[neighborData.field_4188[4].shape] * fs[neighborData.field_4188[5].shape];
                float cc = fs[neighborData.field_4188[6].shape] * fs[neighborData.field_4188[7].shape];
                this.brightness[translation.firstCorner] = Math.clamp(jb * nb + kb * ob + lb * pb + mb * qb, 0.0F, 1.0F);
                this.brightness[translation.secondCorner] = Math.clamp(jb * rb + kb * sb + lb * tb + mb * ub, 0.0F, 1.0F);
                this.brightness[translation.thirdCorner] = Math.clamp(jb * vb + kb * wb + lb * xb + mb * yb, 0.0F, 1.0F);
                this.brightness[translation.fourthCorner] = Math.clamp(jb * zb + kb * ac + lb * bc + mb * cc, 0.0F, 1.0F);
                int dc = this.getAmbientOcclusionBrightness(l, i, u, db);
                int ec = this.getAmbientOcclusionBrightness(k, i, q, db);
                int fc = this.getAmbientOcclusionBrightness(k, j, y, db);
                int gc = this.getAmbientOcclusionBrightness(l, j, cb, db);
                this.light[translation.firstCorner] = this.getBrightness(dc, ec, fc, gc, nb, ob, pb, qb);
                this.light[translation.secondCorner] = this.getBrightness(dc, ec, fc, gc, rb, sb, tb, ub);
                this.light[translation.thirdCorner] = this.getBrightness(dc, ec, fc, gc, vb, wb, xb, yb);
                this.light[translation.fourthCorner] = this.getBrightness(dc, ec, fc, gc, zb, ac, bc, cc);
            } else {
                float fb = (m + f + t + eb) * 0.25F;
                float gb = (h + f + p + eb) * 0.25F;
                float hb = (h + g + x + eb) * 0.25F;
                float ib = (m + g + bb + eb) * 0.25F;
                this.light[translation.firstCorner] = this.getAmbientOcclusionBrightness(l, i, u, db);
                this.light[translation.secondCorner] = this.getAmbientOcclusionBrightness(k, i, q, db);
                this.light[translation.thirdCorner] = this.getAmbientOcclusionBrightness(k, j, y, db);
                this.light[translation.fourthCorner] = this.getAmbientOcclusionBrightness(l, j, cb, db);
                this.brightness[translation.firstCorner] = fb;
                this.brightness[translation.secondCorner] = gb;
                this.brightness[translation.thirdCorner] = hb;
                this.brightness[translation.fourthCorner] = ib;
            }

            float hc = world.getBrightness(direction, bl);

            for (int ic = 0; ic < this.brightness.length; ic++) {
                this.brightness[ic] = this.brightness[ic] * hc;
            }
        }
    ...
    static class BrightnessCache {
        ...
        public int getInt(BlockState state, BlockRenderView world, BlockPos pos) {
            ...
            int j = WorldRenderer.getLightmapCoordinates(world, state, pos);
            if (this.enabled) {
