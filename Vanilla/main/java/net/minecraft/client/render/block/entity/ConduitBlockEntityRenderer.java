...
public class ConduitBlockEntityRenderer implements BlockEntityRenderer<ConduitBlockEntity> {
    public static final SpriteIdentifier BASE_TEXTURE = new SpriteIdentifier(
        SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/base")
    );
    public static final SpriteIdentifier CAGE_TEXTURE = new SpriteIdentifier(
        SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/cage")
    );
    public static final SpriteIdentifier WIND_TEXTURE = new SpriteIdentifier(
        SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.ofVanilla("entity/conduit/wind")
    );
    ...
    public void render(ConduitBlockEntity conduitBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        ...
        if (!conduitBlockEntity.isActive()) {
            ...
            l = l * l + l;
            matrixStack.push();
            matrixStack.translate(0.5F, 0.3F + l * 0.2F, 0.5F);
            Vector3f vector3f = new Vector3f(0.5F, 1.0F, 0.5F).normalize();
            matrixStack.multiply(new Quaternionf().rotationAxis(k * (float) (Math.PI / 180.0), vector3f));
            ...
            matrixStack.multiply(new Quaternionf().rotationYXZ(n * (float) (Math.PI / 180.0), camera.getPitch() * (float) (Math.PI / 180.0), (float) Math.PI));
            float o = 1.3333334F;
