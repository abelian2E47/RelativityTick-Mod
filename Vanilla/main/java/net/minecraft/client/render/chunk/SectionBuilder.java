    ...
    public SectionBuilder.RenderData build(
        ...
        for (BlockPos blockPos3 : BlockPos.iterate(blockPos, blockPos2)) {
            ...
            if (blockState.getRenderType() == BlockRenderType.MODEL) {
                ...
                BufferBuilder bufferBuilder2 = this.beginBufferBuilding(map, allocatorStorage, renderLayer2);
                matrixStack.push();
                ...
                this.blockRenderManager.renderBlock(blockState, blockPos3, renderRegion, matrixStack, bufferBuilder2, true, random);
                matrixStack.pop();
            }
        }

        for (Entry<RenderLayer, BufferBuilder> entry : map.entrySet()) {
            RenderLayer renderLayer3 = entry.getKey();
            BuiltBuffer builtBuffer = entry.getValue().endNullable();
            if (builtBuffer != null) {
                if (renderLayer3 == RenderLayer.getTranslucent()) {
                    renderData.translucencySortingData = builtBuffer.sortQuads(allocatorStorage.get(RenderLayer.getTranslucent()), vertexSorter);
                }

                renderData.buffers.put(renderLayer3, builtBuffer);
            }
        }
