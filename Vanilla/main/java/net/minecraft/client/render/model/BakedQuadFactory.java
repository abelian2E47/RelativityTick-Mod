...
public class BakedQuadFactory {
    ...
    public static ModelElementTexture uvLock(ModelElementTexture texture, Direction orientation, AffineTransformation rotation) {
        ...
        Vector3f vector3f = matrix3f.transform(new Vector3f(MathHelper.cos(v), MathHelper.sin(v), 0.0F));
        int w = Math.floorMod(-((int)Math.round(Math.toDegrees(Math.atan2(vector3f.y(), vector3f.x())) / 90.0)) * 90, 360);
