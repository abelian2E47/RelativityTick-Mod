            ...
            try {
                list.stream().filter(BlockPropertiesPredicate.parse(stateManager, key)).forEach(state -> {
                    ...
                    if (groupableModel != null && groupableModel != multipartUnbakedModel) {
                        String string = this.variantMap.entrySet().stream().filter(entry -> entry.getValue() == groupableModel).findFirst().get().getKey();
        ...
        protected Map<String, WeightedUnbakedModel> variantsFromJson(JsonDeserializationContext context, JsonObject object) {
            ...
            if (object.has("variants")) {
                ...
                for (Entry<String, JsonElement> entry : jsonObject.entrySet()) {
                    map.put(entry.getKey(), context.deserialize(entry.getValue(), WeightedUnbakedModel.class));
                }
            }
