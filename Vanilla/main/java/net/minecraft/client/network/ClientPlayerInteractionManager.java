...
public class ClientPlayerInteractionManager {
    ...
    public boolean attackBlock(BlockPos pos, Direction direction) {
        ...
        if (this.gameMode.isCreative()) {
            ...
            this.sendSequencedPacket(this.client.world, sequence -> {
                ...
                if (bl && blockState2.calcBlockBreakingDelta(this.client.player, this.client.player.getWorld(), pos) >= 1.0F) {
                    this.breakBlock(pos);
                } else {
    ...
    public void cancelBlockBreaking() {
        if (this.breakingBlock) {
            ...
            this.networkHandler
                .sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, this.currentBreakingPos, Direction.DOWN));
    ...
    public boolean updateBlockBreakingProgress(BlockPos pos, Direction direction) {
        ...
        if (this.isCurrentlyBreaking(pos)) {
            ...
            this.currentBreakingProgress = this.currentBreakingProgress
                + blockState2.calcBlockBreakingDelta(this.client.player, this.client.player.getWorld(), pos);
