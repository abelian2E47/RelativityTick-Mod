...
public class ClientPlayNetworkHandler extends ClientCommonNetworkHandler implements ClientPlayPacketListener, TickablePacketListener {
        ...
        if (entity != null) {
            if (entity.isLogicalSideForUpdatingMovement()) {
                ...
                if (packet.isPositionChanged()) {
                    ...
                    Vec3d vec3d2 = trackedPosition2.withDelta(packet.getDeltaX(), packet.getDeltaY(), packet.getDeltaZ());
                    trackedPosition2.setPos(vec3d2);
                    ...
                    entity.updateTrackedPositionAndAngles(vec3d2.getX(), vec3d2.getY(), vec3d2.getZ(), f, g, 3);
    ...
    private static boolean setPosition(PlayerPosition pos, Set<PositionFlag> flags, Entity entity, boolean bl) {
        ...
        if (bl && !bl2) {
            entity.updateTrackedPositionAndAngles(
                playerPosition2.position().getX(),
                playerPosition2.position().getY(),
                playerPosition2.position().getZ(),
                playerPosition2.yaw(),
    ...
    public void onChunkBiomeData(ChunkBiomeDataS2CPacket packet) {
        ...
        for (ChunkBiomeDataS2CPacket.Serialized serialized2 : packet.chunkBiomeData()) {
            this.world.resetChunkColor(new ChunkPos(serialized2.pos().x, serialized2.pos().z));
        }

        for (ChunkBiomeDataS2CPacket.Serialized serialized3 : packet.chunkBiomeData()) {
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    for (int k = this.world.getBottomSectionCoord(); k <= this.world.getTopSectionCoord(); k++) {
                        this.client.worldRenderer.scheduleChunkRender(serialized3.pos().x + i, k, serialized3.pos().z + j);
                    }
                }
            }
        }
    ...
    public void onEntityEquipmentUpdate(EntityEquipmentUpdateS2CPacket packet) {
        ...
        if (this.world.getEntityById(packet.getEntityId()) instanceof LivingEntity livingEntity) {
            packet.getEquipmentList().forEach(pair -> livingEntity.equipStack(pair.getFirst(), pair.getSecond()));
        }
    }
    ...
    public void onPlayerList(PlayerListS2CPacket packet) {
        ...
        for (PlayerListS2CPacket.Entry entry2 : packet.getEntries()) {
            PlayerListEntry playerListEntry2 = this.playerListEntries.get(entry2.profileId());
            if (playerListEntry2 == null) {
                LOGGER.warn("Ignoring player info update for unknown player {} ({})", entry2.profileId(), packet.getActions());
            } else {
                for (PlayerListS2CPacket.Action action : packet.getActions()) {
                    this.handlePlayerListAction(action, entry2, playerListEntry2);
                }
            }
        }
    }
    ...
    public void onPlaySoundFromEntity(PlaySoundFromEntityS2CPacket packet) {
        ...
        if (entity != null) {
            this.client
                .world
                .playSoundFromEntity(
                    this.client.player, entity, packet.getSound(), packet.getCategory(), packet.getVolume(), packet.getPitch(), packet.getSeed()
                );
        }
    }
    ...
    public void onTeam(TeamS2CPacket packet) {
        ...
        if (operation2 == TeamS2CPacket.Operation.ADD) {
            ...
            for (String string2 : packet.getPlayerNames()) {
                this.scoreboard.removeScoreHolderFromTeam(string2, team);
            }
        }
