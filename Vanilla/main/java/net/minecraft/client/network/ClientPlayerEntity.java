...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tickMovement() {
        ...
        if (this.input.playerInput.jump() && !bl10 && !bl && !this.isClimbing() && this.checkGliding()) {
        ...
        if (playerAbilities.flying && this.isCamera()) {
            ...
            if (j != 0) {
                this.setVelocity(this.getVelocity().add(0.0, j * playerAbilities.getFlySpeed() * 3.0F, 0.0));
            }
        }
    ...
    private void tickNausea(boolean fromPortalEffect) {
        ...
        if (fromPortalEffect && this.portalManager != null && this.portalManager.isInPortal()) {
            ...
            if (this.nauseaIntensity == 0.0F) {
                this.client
                    .getSoundManager()
                    .play(PositionedSoundInstance.ambient(SoundEvents.BLOCK_PORTAL_TRIGGER, this.random.nextFloat() * 0.4F + 0.8F, 0.25F));
            }
                ...
                if (blockState.getCollisionShape(this.getWorld(), blockPos, shapeContext).isEmpty()) {
                    ...
                    if (blockState2.getCollisionShape(this.getWorld(), blockPos, shapeContext).isEmpty()) {
                        ...
                        if (this.hasStatusEffect(StatusEffects.JUMP_BOOST)) {
                            o += (this.getStatusEffect(StatusEffects.JUMP_BOOST).getAmplifier() + 1) * 0.75F;
                        }
                        ...
                        Iterator<Box> iterator = StreamSupport.stream(iterable.spliterator(), false)
                            ...
                            if (box2.intersects(vec3d10, vec3d11) || box2.intersects(vec3d12, vec3d13)) {
                                ...
                                for (int t = 1; t < o; t++) {
                                    ...
                                    if (!(voxelShape = blockState3.getCollisionShape(this.getWorld(), blockPos3, shapeContext)).isEmpty()) {
                                        s = (float)voxelShape.getMax(Direction.Axis.Y) + blockPos3.getY();
                                        if (s - this.getY() > o) {
                                            return;
                                        }
                                    }
