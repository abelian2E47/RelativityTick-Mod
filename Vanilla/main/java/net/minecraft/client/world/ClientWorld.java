...
public class ClientWorld extends World {
    ...
    public void addFireworkParticle(
        ...
        if (explosions.isEmpty()) {
            ...
            this.client
                .particleManager
                .addParticle(
                    new FireworksSparkParticle.FireworkParticle(this, x, y, z, velocityX, velocityY, velocityZ, this.client.particleManager, explosions)
                );
        }
    }
    ...
    public int getSkyColor(Vec3d cameraPos, float tickDelta) {
        ...
        if (m > 0.0F) {
            float n = 0.2F;
            float o = m * 0.75F;
            int p = ColorHelper.scaleRgb(ColorHelper.grayscale(i), 0.2F);
            i = ColorHelper.lerp(o, i, p);
        }

        int q = this.getLightningTicksLeft();
        if (q > 0) {
            float r = Math.min(q - tickDelta, 1.0F);
            r *= 0.45F;
            i = ColorHelper.lerp(r, i, ColorHelper.getArgb(204, 204, 255));
        }
