...
public class BiomeColorCache {
    ...
    private BiomeColorCache.Colors getColorArray(int chunkX, int chunkZ) {
        ...
        try {
            BiomeColorCache.Colors colors2 = this.colors.get(l);
            if (colors2 != null) {
                return colors2;
            }

            BiomeColorCache.Colors colors3 = new BiomeColorCache.Colors();
            if (this.colors.size() >= 256) {
                BiomeColorCache.Colors colors4 = this.colors.removeFirst();
                if (colors4 != null) {
                    colors4.setNeedsCacheRefresh();
                }
            }

            this.colors.put(l, colors3);
            return colors3;
        } finally {
            this.lock.writeLock().unlock();
        }
    ...
    static class Colors {
        ...
        public int[] get(int y) {
            ...
            try {
                return this.colors.computeIfAbsent(y, yx -> this.createDefault());
            } finally {
                this.lock.writeLock().unlock();
            }
        }
