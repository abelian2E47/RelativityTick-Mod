...
public class InGameHud {
            ...
            if (this.client.player.isUsingSpyglass()) {
                ...
                for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
                    ...
                    if (equippableComponent != null && equippableComponent.slot() == equipmentSlot && equippableComponent.cameraOverlay().isPresent()) {
                        this.renderOverlay(
                            context, equippableComponent.cameraOverlay().get().withPath(overlayTexture -> "textures/" + overlayTexture + ".png"), 1.0F
                        );
                    }
                }
            }
    ...
    private void renderOverlayMessage(DrawContext context, RenderTickCounter tickCounter) {
        ...
        if (this.overlayMessage != null && this.overlayRemaining > 0) {
            ...
            if (i > 8) {
                ...
                context.drawTextWithBackground(textRenderer, this.overlayMessage, -l / 2, -4, l, j);
            ...
            for (StatusEffectInstance statusEffectInstance : Ordering.natural().reverse().sortedCopy(collection)) {
                ...
                if (statusEffectInstance.shouldShowIcon()) {
                    ...
                    list.add(() -> {
                        ...
                        context.drawSpriteStretched(RenderLayer::getGuiTextured, sprite, o + 3, p + 3, 18, 18, kx);
                    });
                }
            }
    ...
    private void renderHotbar(DrawContext context, RenderTickCounter tickCounter) {
        ...
        if (playerEntity != null) {
            ...
            if (!itemStack.isEmpty()) {
                int p = context.getScaledWindowHeight() - 16 - 3;
                if (arm == Arm.LEFT) {
                    this.renderHotbarItem(context, i - 91 - 26, p, tickCounter, playerEntity, itemStack, l++);
                } else {
                    this.renderHotbarItem(context, i + 91 + 10, p, tickCounter, playerEntity, itemStack, l++);
                }
            }
            ...
            if (this.client.options.getAttackIndicator().getValue() == AttackIndicator.HOTBAR) {
                float f = this.client.player.getAttackCooldownProgress(0.0F);
                if (f < 1.0F) {
                    int q = context.getScaledWindowHeight() - 20;
                    int r = i + 91 + 6;
                    if (arm == Arm.RIGHT) {
                        r = i - 91 - 22;
                    }
                    ...
                    int s = (int)(f * 19.0F);
                    context.drawGuiTexture(RenderLayer::getGuiTextured, HOTBAR_ATTACK_INDICATOR_BACKGROUND_TEXTURE, r, q, 18, 18);
                    context.drawGuiTexture(RenderLayer::getGuiTextured, HOTBAR_ATTACK_INDICATOR_PROGRESS_TEXTURE, 18, 18, 0, 18 - s, r, q + 18 - s, 18, s);
                }
            }
        }
    }
        ...
        if (this.client.isDemo()) {
            ...
            if (this.client.world.getTime() >= 120500L) {
                ...
                text = Text.translatable(
                    "demo.remainingTime",
                    StringHelper.formatTicks((int)(120500L - this.client.world.getTime()), this.client.world.getTickManager().getTickRate())
                );
            }
    ...
    private void renderScoreboardSidebar(DrawContext drawContext, ScoreboardObjective objective) {
        ...
        drawContext.drawText(this.getTextRenderer(), text, q + l / 2 - i / 2, u - 9, Colors.WHITE, false);
    ...
    private void renderHealthBar(
        ...
        for (int l = i + j - 1; l >= 0; l--) {
            ...
            if (q < lastHealth) {
                boolean bl5 = q + 1 == lastHealth;
                this.drawHeart(context, heartType, o, p, bl, false, bl5);
            }
        }
    }
    ...
    private void renderNauseaOverlay(DrawContext context, float nauseaStrength) {
        ...
        context.drawTexture(
            identifier -> RenderLayer.getGuiNauseaOverlay(), NAUSEA_TEXTURE, 0, 0, 0.0F, 0.0F, i, j, i, j, ColorHelper.fromFloats(1.0F, g, h, k)
        );
        context.getMatrices().pop();
    }
    ...
    public void renderAutosaveIndicator(DrawContext context, RenderTickCounter tickCounter) {
        if (this.client.options.getShowAutosaveIndicator().getValue() && (this.autosaveIndicatorAlpha > 0.0F || this.lastAutosaveIndicatorAlpha > 0.0F)) {
            int i = MathHelper.floor(
                255.0F
                    * MathHelper.clamp(MathHelper.lerp(tickCounter.getLastDuration(), this.lastAutosaveIndicatorAlpha, this.autosaveIndicatorAlpha), 0.0F, 1.0F)
            );
            if (i > 8) {
                TextRenderer textRenderer = this.getTextRenderer();
