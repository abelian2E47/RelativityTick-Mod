    ...
    public void render(DrawContext context, int currentTick, int mouseX, int mouseY, boolean focused) {
        if (!this.isChatHidden()) {
            ...
            if (j > 0) {
                            ...
                            if (u > 3) {
                                ...
                                if (messageIndicator != null) {
                                    ...
                                    if (s == n && messageIndicator.icon() != null) {
                                        int ab = this.getIndicatorX(visible);
                                        int bb = y + 9;
                                        this.drawIndicatorIcon(context, ab, bb, messageIndicator.icon());
                                    }
                                }
                ...
                long cb = this.client.getMessageHandler().getUnprocessedMessageCount();
                if (cb > 0L) {
                    int db = (int)(128.0 * d);
                    int eb = (int)(255.0 * e);
                    context.getMatrices().push();
                    context.getMatrices().translate(0.0F, m, 0.0F);
                    context.fill(-2, 0, k + 4, 9, eb << 24);
                    context.getMatrices().translate(0.0F, 0.0F, 50.0F);
                    context.drawTextWithShadow(this.client.textRenderer, Text.translatable("chat.queue", cb), 0, 1, 16777215 + (db << 24));
                    context.getMatrices().pop();
                }

                if (focused) {
                    int fb = this.getLineHeight();
                    int gb = j * fb;
                    int hb = q * fb;
                    int ib = this.scrolledLines * hb / j - m;
                    int jb = hb * hb / gb;
                    if (gb != hb) {
                        int kb = ib > 0 ? 170 : 96;
                        int lb = this.hasUnreadNewMessages ? 13382451 : 3355562;
                        int mb = k + 4;
                        context.fill(mb, -ib, mb + 2, -ib - jb, 100, lb + (kb << 24));
                        context.fill(mb + 2, -ib, mb + 1, -ib - jb, 100, 13421772 + (kb << 24));
                    }
                }
