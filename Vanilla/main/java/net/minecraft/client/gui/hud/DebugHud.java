...
public class DebugHud {
    ...
    public void render(DrawContext context) {
        ...
        if (this.packetSizeAndPingChartsVisible) {
            int l = context.getScaledWindowWidth();
            int m = l / 2;
            if (!this.client.isInSingleplayer()) {
                this.packetSizeChart.render(context, 0, this.packetSizeChart.getWidth(m));
            }

            int n = this.pingChart.getWidth(m);
            this.pingChart.render(context, l - n, n);
    ...
    private void drawText(DrawContext context, List<String> text, boolean left) {
        ...
        for (int n = 0; n < text.size(); n++) {
            String string2 = text.get(n);
            if (!Strings.isNullOrEmpty(string2)) {
                int o = this.textRenderer.getWidth(string2);
                int p = left ? 2 : context.getScaledWindowWidth() - 2 - o;
                int q = 2 + i * n;
                context.drawText(this.textRenderer, string2, p, q, 14737632, false);
            }
        }
    }
    ...
    protected List<String> getLeftText() {
        ...
        if (integratedServer != null) {
            ...
            String string4 = bl ? "-" : String.format(Locale.ROOT, "%.1f", tickManager.getMillisPerTick());
            string5 = String.format(
                Locale.ROOT, "Integrated server @ %.1f/%s ms%s, %.0f tx, %.0f rx", integratedServer.getAverageTickTime(), string4, string, f, g
            );
        } else {
            string5 = String.format(Locale.ROOT, "\"%s\" server%s, %.0f tx, %.0f rx", clientPlayNetworkHandler.getBrand(), string, f, g);
        }
        ...
        String string11 = switch (direction) {
        ...
        String string12 = this.getServerWorldDebugString();
        if (string12 != null) {
            list.add(string12);
        }
        ...
        if (worldChunk.isEmpty()) {
            ...
            for (Heightmap.Type type2 : Heightmap.Type.values()) {
                if (type2.isStoredServerSide()) {
                    stringBuilder.append(" ").append(HEIGHT_MAP_TYPES.get(type2)).append(": ");
                    if (worldChunk2 != null) {
    ...
    protected List<String> getRightText() {
        ...
        if (this.fluidHit.getType() == HitResult.Type.BLOCK) {
            ...
            for (Entry<Property<?>, Comparable<?>> entry2 : fluidState.getEntries().entrySet()) {
