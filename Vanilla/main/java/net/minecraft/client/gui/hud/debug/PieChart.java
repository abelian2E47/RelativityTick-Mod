        ...
        if (this.profileResult != null) {
            ...
            context.draw(vertexConsumers -> {
                ...
                for (ProfilerTiming profilerTimingx : list) {
                    ...
                    for (int ox = kx; ox > 0; ox--) {
                        float px = (float)((d + profilerTimingx.parentSectionUsagePercentage * ox / kx) * (float) (Math.PI * 2) / 100.0);
                        float qx = MathHelper.sin(px) * 105.0F;
                        float rx = MathHelper.cos(px) * 105.0F * 0.5F;
                        float sx = (float)((d + profilerTimingx.parentSectionUsagePercentage * (ox - 1) / kx) * (float) (Math.PI * 2) / 100.0);
                        float tx = MathHelper.sin(sx) * 105.0F;
                        float u = MathHelper.cos(sx) * 105.0F * 0.5F;
                        if (!((rx + u) / 2.0F > 0.0F)) {
                            vertexConsumer.vertex(entry, i + qx, p - rx, 10.0F).color(mx);
                            vertexConsumer.vertex(entry, i + qx, p - rx + 10.0F, 10.0F).color(mx);
                            vertexConsumer.vertex(entry, i + tx, p - u + 10.0F, 10.0F).color(mx);
                            vertexConsumer.vertex(entry, i + tx, p - u, 10.0F).color(mx);
                        }
                    }
    ...
    public void select(int index) {
        if (this.profileResult != null) {
            ...
            if (!list.isEmpty()) {
                ...
                } else if (--index < list.size() && !"unspecified".equals(list.get(index).name)) {
