...
public class DrawContext {
    ...
    public void drawTooltip(TextRenderer textRenderer, List<Text> text, int x, int y, @Nullable Identifier texture) {
        this.drawTooltip(
            textRenderer, text.stream().map(Text::asOrderedText).map(TooltipComponent::of).toList(), x, y, HoveredTooltipPositioner.INSTANCE, texture
        );
    }
    ...
    private void drawTooltip(
        ...
        if (!components.isEmpty()) {
            ...
            for (int s = 0; s < components.size(); s++) {
                TooltipComponent tooltipComponent3 = components.get(s);
                tooltipComponent3.drawItems(textRenderer, n, q, l, m, this);
                q += tooltipComponent3.getHeight(textRenderer) + (s == 0 ? 2 : 0);
            }
