...
public class BookEditScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0) {
            ...
            BookEditScreen.PageContent pageContent = this.getPageContent();
            int i = pageContent.getCursorPosition(
                this.textRenderer, this.screenPositionToAbsolutePosition(new BookEditScreen.Position((int)mouseX, (int)mouseY))
            );
            if (i >= 0) {
    ...
    private BookEditScreen.PageContent createPageContent() {
        ...
        textHandler.wrapLines(string, 114, Style.EMPTY, true, (style, start, end) -> {
            ...
            int jx = ix * 9;
            BookEditScreen.Position position = this.absolutePositionToScreenPosition(new BookEditScreen.Position(0, jx));
            intList.add(start);
            list.add(new BookEditScreen.Line(style, string2x, position.x, position.y));
        });
