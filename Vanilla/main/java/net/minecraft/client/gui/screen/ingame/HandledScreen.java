...
public abstract class HandledScreen<T extends ScreenHandler> extends Screen implements ScreenHandlerProvider<T> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!this.touchDropReturningStack.isEmpty()) {
            ...
            int m = this.touchDropOriginSlot.x - this.touchDropX;
            int n = this.touchDropOriginSlot.y - this.touchDropY;
            int o = this.touchDropX + (int)(m * f);
            int p = this.touchDropY + (int)(n * f);
    ...
    protected void drawSlot(DrawContext context, Slot slot) {
        ...
        if (!bl2) {
            ...
            int n = slot.x + slot.y * this.backgroundWidth;
            if (slot.disablesDynamicDisplay()) {
                context.drawItemWithoutEntity(itemStack, i, j, n);
            } else {
                context.drawItem(itemStack, i, j, n);
            }
