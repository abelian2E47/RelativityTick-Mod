...
public class MultiplayerServerListWidget extends AlwaysSelectedEntryListWidget<MultiplayerServerListWidget.Entry> {
    ...
    static final ThreadPoolExecutor SERVER_PINGER_THREAD_POOL = new ScheduledThreadPoolExecutor(
        5,
        new ThreadFactoryBuilder().setNameFormat("Server Pinger #%d").setDaemon(true).setUncaughtExceptionHandler(new UncaughtExceptionLogger(LOGGER)).build()
    );
    ...
    public class ServerEntry extends MultiplayerServerListWidget.Entry {
        ...
        public void render(
            ...
            if (this.server.getStatus() == ServerInfo.Status.PINGING) {
                int j = (int)(Util.getMeasuringTimeMs() / 100L + index * 2 & 7L);
                if (j > 4) {
                    j = 8 - j;
                }
            ...
            int k = x + entryWidth - 10 - 5;
            if (this.statusIconTexture != null) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, this.statusIconTexture, k, y, 10, 8);
            }
            ...
            Text text = this.server.getStatus() == ServerInfo.Status.INCOMPATIBLE
                ? this.server.version.copy().formatted(Formatting.RED)
                : this.server.playerCountLabel;
            int l = this.client.textRenderer.getWidth(text);
            int m = k - l - 5;
            context.drawTextWithShadow(this.client.textRenderer, text, m, y + 1, Colors.GRAY);
            if (this.statusTooltipText != null && mouseX >= k && mouseX <= k + 10 && mouseY >= y && mouseY <= y + 8) {
                this.screen.setTooltip(this.statusTooltipText);
            } else if (this.playerListSummary != null && mouseX >= m && mouseX <= m + l && mouseY >= y && mouseY <= y - 1 + 9) {
        ...
        private boolean uploadFavicon(@Nullable byte[] bytes) {
            if (bytes == null) {
                ...
                try {
                    ...
                    MultiplayerServerListWidget.LOGGER.error("Invalid icon for server {} ({})", this.server.name, this.server.address, throwable);
                    return false;
                }
            }
            ...
            switch (this.server.getStatus()) {
                ...
                default:
                    ...
                    if (this.server.players != null) {
                        mutableText.append(ScreenTexts.SENTENCE_SEPARATOR);
                        mutableText.append(
                            Text.translatable("multiplayer.status.player_count.narration", this.server.players.online(), this.server.players.max())
                        );
                        mutableText.append(ScreenTexts.SENTENCE_SEPARATOR);
