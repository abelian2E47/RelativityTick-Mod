        ...
        class Entry extends AlwaysSelectedEntryListWidget.Entry<StatsScreen.EntityStatsListWidget.Entry> {
            ...
            public void render(
                ...
                context.drawTextWithShadow(
                    StatsScreen.this.textRenderer, this.killedText, x + 2 + 10, y + 1 + 9, this.killedAny ? Colors.ALTERNATE_WHITE : Colors.GRAY
                );
                context.drawTextWithShadow(
    ...
    class ItemStatsListWidget extends AlwaysSelectedEntryListWidget<StatsScreen.ItemStatsListWidget.Entry> {
        ...
        protected void renderHeader(DrawContext context, int x, int y) {
            ...
            if (this.selectedStatType != null) {
                int j = this.getIconX(this.getHeaderIndex(this.selectedStatType)) - 36;
                Identifier identifier2 = this.listOrder == 1 ? StatsScreen.SORT_UP_TEXTURE : StatsScreen.SORT_DOWN_TEXTURE;
                context.drawGuiTexture(RenderLayer::getGuiTextured, identifier2, x + j, y + 1, 18, 18);
            }
        ...
        protected void renderDecorations(DrawContext context, int mouseX, int mouseY) {
            if (mouseY >= this.getY() && mouseY <= this.getBottom()) {
                ...
                if (entry != null) {
                    ...
                    context.drawTooltip(
                        StatsScreen.this.textRenderer, item.getName(), mouseX, mouseY, item.getComponents().get(DataComponentTypes.TOOLTIP_STYLE)
                    );
                } else {
                    Text text = null;
                    int j = mouseX - i;
            ...
            public void render(
                ...
                if (StatsScreen.this.itemStats != null) {
                    ...
                    for (int j = 0; j < StatsScreen.this.itemStats.itemStatTypes.size(); j++) {
                        this.render(
                            context,
                            StatsScreen.this.itemStats.itemStatTypes.get(j).getOrCreateStat(this.item),
                            x + ItemStatsListWidget.this.getIconX(j + StatsScreen.this.itemStats.blockStatTypes.size()),
                            y + entryHeight / 2 - 9 / 2,
                            index % 2 == 0
                        );
                    }
