...
public abstract class AbstractSignEditScreen extends Screen {
    ...
    private void renderSignText(DrawContext context) {
        ...
        for (int r = 0; r < this.messages.length; r++) {
            String string2 = this.messages[r];
            if (string2 != null && r == this.currentRow && j >= 0) {
                ...
                if (bl && j < string2.length()) {
                    context.fill(t, m - 1, t + 1, m + this.blockEntity.getTextLineHeight(), ColorHelper.fullAlpha(i));
                }

                if (k != j) {
                    int u = Math.min(j, k);
                    int v = Math.max(j, k);
                    int w = this.textRenderer.getWidth(string2.substring(0, u)) - this.textRenderer.getWidth(string2) / 2;
                    int x = this.textRenderer.getWidth(string2.substring(0, v)) - this.textRenderer.getWidth(string2) / 2;
                    int y = Math.min(w, x);
                    int z = Math.max(w, x);
                    context.fill(RenderLayer.getGuiTextHighlight(), y, m, z, m + this.blockEntity.getTextLineHeight(), Colors.BLUE);
                }
            }
        }
    }
