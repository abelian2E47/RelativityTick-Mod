            ...
            this.client
                .setScreen(
                    new ExperimentsScreen(
                        this, pair.getSecond(), resourcePackManager -> this.applyDataPacks(resourcePackManager, false, this::openExperimentsScreen)
                    )
                );
        }
    }
            ...
            try (Stream<Path> stream = Files.walk(this.dataPackTempDir)) {
                stream.sorted(Comparator.reverseOrder()).forEach(path -> {
                    try {
                        ...
                        LOGGER.warn("Failed to remove temporary file {}", path, iOExceptionx);
                    }
                });
            } catch (IOException iOException) {
            ...
            CreateWorldScreen.this.worldCreator
                .addListener(
                    creator -> this.worldNameField
                        .setTooltip(
                            Tooltip.of(
                                Text.translatable("selectWorld.targetFolder", Text.literal(creator.getWorldDirectoryName()).formatted(Formatting.ITALIC))
                            )
                        )
                );
            ...
            if (!SharedConstants.getGameVersion().isStable()) {
                adder.add(
                    ButtonWidget.builder(
                            ...
                            button -> CreateWorldScreen.this.openExperimentsScreen(
                                CreateWorldScreen.this.worldCreator.getGeneratorOptionsHolder().dataConfiguration()
                            )
                        )
                        .width(210)
                        .build()
