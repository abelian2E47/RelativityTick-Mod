...
public class CreativeInventoryScreen extends HandledScreen<CreativeInventoryScreen.CreativeScreenHandler> {
        ...
        if (actionType != SlotActionType.THROW || this.client.player.canDropItems()) {
            ...
            if (slot == null && selectedTab.getType() != ItemGroup.Type.INVENTORY && actionType != SlotActionType.QUICK_CRAFT) {
                if (!this.handler.getCursorStack().isEmpty() && this.lastClickOutsideBounds) {
                    ...
                    if (button == 1) {
                        ItemStack itemStack9 = this.handler.getCursorStack().split(1);
                        this.client.player.dropItem(itemStack9, true);
                        this.client.interactionManager.dropCreativeStack(itemStack9);
                    }
                }
            } else {
                ...
                if (slot == this.deleteItemSlot && bl) {
                    ...
                    if (actionType == SlotActionType.SWAP) {
                        if (!itemStack4.isEmpty()) {
                            this.client.player.getInventory().setStack(button, itemStack4.copyWithCount(itemStack4.getMaxCount()));
                            this.client.player.playerScreenHandler.sendContentUpdates();
                        }
                    ...
                    if (actionType == SlotActionType.CLONE) {
                        if (this.handler.getCursorStack().isEmpty() && slot.hasStack()) {
                            ...
                            this.handler.setCursorStack(itemStack5.copyWithCount(itemStack5.getMaxCount()));
                        }
                    ...
                    if (actionType == SlotActionType.THROW) {
                        if (!itemStack4.isEmpty()) {
                            ItemStack itemStack6 = itemStack4.copyWithCount(button == 0 ? 1 : itemStack4.getMaxCount());
                    ...
                    if (!itemStack3.isEmpty() && !itemStack4.isEmpty() && ItemStack.areItemsAndComponentsEqual(itemStack3, itemStack4)) {
                        if (button == 0) {
                            if (bl) {
                        ...
                        int j = bl ? itemStack4.getMaxCount() : itemStack4.getCount();
                        this.handler.setCursorStack(itemStack4.copyWithCount(j));
                    } else if (button == 0) {
                        this.handler.setCursorStack(ItemStack.EMPTY);
                    ...
                    ItemStack itemStack7 = slot == null ? ItemStack.EMPTY : this.handler.getSlot(slot.id).getStack();
                    ...
                    if (ScreenHandler.unpackQuickCraftStage(button) == 2) {
                        ...
                        if (actionType == SlotActionType.THROW && !itemStack7.isEmpty() && !this.handler.getCursorStack().isEmpty()) {
                            int l = button == 0 ? 1 : itemStack7.getCount();
                            ItemStack itemStack8 = itemStack7.copyWithCount(l);
                            itemStack7.decrement(l);
    ...
    private void setSelectedTab(ItemGroup group) {
        ...
        if (selectedTab.getType() == ItemGroup.Type.INVENTORY) {
            ...
            for (int k = 0; k < screenHandler.slots.size(); k++) {
                int o;
                int p;
                if (k >= 5 && k < 9) {
                    int l = k - 5;
                ...
                Slot slot = new CreativeInventoryScreen.CreativeSlot(screenHandler.slots.get(k), k, o, p);
