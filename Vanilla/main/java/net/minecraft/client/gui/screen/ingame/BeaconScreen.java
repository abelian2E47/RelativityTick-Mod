...
public class BeaconScreen extends HandledScreen<BeaconScreenHandler> {
    ...
    protected void init() {
        ...
        int m = 3;
        int n = BeaconBlockEntity.EFFECTS_BY_LEVEL.get(3).size() + 1;
        int o = n * 22 + (n - 1) * 2;

        for (int p = 0; p < n - 1; p++) {
            RegistryEntry<StatusEffect> registryEntry2 = BeaconBlockEntity.EFFECTS_BY_LEVEL.get(3).get(p);
            BeaconScreen.EffectButtonWidget effectButtonWidget2 = new BeaconScreen.EffectButtonWidget(
                this.x + 167 + p * 24 - o / 2, this.y + 47, registryEntry2, false, 3
            );
            effectButtonWidget2.active = false;
            this.addButton(effectButtonWidget2);
        }

        RegistryEntry<StatusEffect> registryEntry3 = BeaconBlockEntity.EFFECTS_BY_LEVEL.get(0).get(0);
        BeaconScreen.EffectButtonWidget effectButtonWidget3 = new BeaconScreen.LevelTwoEffectButtonWidget(
            this.x + 167 + (n - 1) * 24 - o / 2, this.y + 47, registryEntry3
        );
    ...
    class DoneButtonWidget extends BeaconScreen.IconButtonWidget {
        ...
        public void onPress() {
            BeaconScreen.this.client
                .getNetworkHandler()
                .sendPacket(
                    new UpdateBeaconC2SPacket(Optional.ofNullable(BeaconScreen.this.primaryEffect), Optional.ofNullable(BeaconScreen.this.secondaryEffect))
                );
            BeaconScreen.this.client.player.closeHandledScreen();
        }
