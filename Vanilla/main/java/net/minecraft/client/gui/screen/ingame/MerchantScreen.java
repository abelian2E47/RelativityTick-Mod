...
public class MerchantScreen extends HandledScreen<MerchantScreenHandler> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!tradeOfferList.isEmpty()) {
            ...
            TradeOffer tradeOffer2 = tradeOfferList.get(o);
        ...
        public void renderTooltip(DrawContext context, int x, int y) {
            if (this.hovered && MerchantScreen.this.handler.getRecipes().size() > this.index + MerchantScreen.this.indexStartOffset) {
                if (x < this.getX() + 20) {
                    ...
                    ItemStack itemStack2 = MerchantScreen.this.handler
                        .getRecipes()
                        .get(this.index + MerchantScreen.this.indexStartOffset)
                        .getDisplayedSecondBuyItem();
                    if (!itemStack2.isEmpty()) {
                        context.drawItemTooltip(MerchantScreen.this.textRenderer, itemStack2, x, y);
                    }
                } else if (x > this.getX() + 65) {
                    ItemStack itemStack3 = MerchantScreen.this.handler.getRecipes().get(this.index + MerchantScreen.this.indexStartOffset).getSellItem();
                    context.drawItemTooltip(MerchantScreen.this.textRenderer, itemStack3, x, y);
                }
            }
        }
    }
