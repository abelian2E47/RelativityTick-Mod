    ...
    private static Identifier getTextureIdInternal(String id, String image) {
        ...
        if (realmsTexture != null && realmsTexture.image().equals(image)) {
            ...
            if (nativeImage == null) {
                ...
                Identifier identifier2 = Identifier.of("realms", "dynamic/" + id);
                MinecraftClient.getInstance().getTextureManager().registerTexture(identifier2, new NativeImageBackedTexture(nativeImage));
                TEXTURES.put(id, new RealmsTextureManager.RealmsTexture(image, identifier2));
                return identifier2;
            }
        }
    }
