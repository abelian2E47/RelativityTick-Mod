                    ...
                    button -> this.client
                        .setScreen(
                            new RealmsSlotOptionsScreen(
                                this, this.server.slots.get(this.server.activeSlot).clone(), this.server.worldType, this.server.activeSlot
                            )
                        )
                )
                    ...
                    button -> this.client
                        .setScreen(
                            RealmsCreateWorldScreen.resetWorld(
                                this, this.server.clone(), () -> this.client.execute(() -> this.client.setScreen(this.getNewScreen()))
                            )
                        )
                )
            ...
            if (this.server.daysLeft < 7) {
                this.drawServerState(context, x, y, mouseX, mouseY, EXPIRES_SOON_STATUS_TEXTURE, () -> {
                    if (this.server.daysLeft <= 0) {
                        ...
                        return this.server.daysLeft == 1 ? EXPIRES_IN_A_DAY_TEXT : Text.translatable("mco.selectServer.expires.days", this.server.daysLeft);
                    }
                });
            } else {
