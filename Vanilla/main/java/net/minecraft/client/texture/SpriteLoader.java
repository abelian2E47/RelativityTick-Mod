...
public class SpriteLoader {
    ...
    public SpriteLoader.StitchResult stitch(List<SpriteContents> sprites, int mipLevel, Executor executor) {
        ...
        int q = Math.max(textureStitcher.getWidth(), this.width);
        int r = Math.max(textureStitcher.getHeight(), this.height);
        Map<Identifier, Sprite> map = this.collectStitchedSprites(textureStitcher, q, r);
        Sprite sprite = map.get(MissingSprite.getMissingSpriteId());
        CompletableFuture<Void> completableFuture;
        if (o > 0) {
            completableFuture = CompletableFuture.runAsync(() -> map.values().forEach(spritex -> spritex.getContents().generateMipmaps(o)), executor);
        } else {
            completableFuture = CompletableFuture.completedFuture(null);
        }

        return new SpriteLoader.StitchResult(q, r, o, sprite, map, completableFuture);
    }
    ...
    public CompletableFuture<SpriteLoader.StitchResult> load(
        ...
        return CompletableFuture.<List<Function<SpriteOpener, SpriteContents>>>supplyAsync(
                () -> AtlasLoader.of(resourceManager, path).loadSources(resourceManager), executor
            )
