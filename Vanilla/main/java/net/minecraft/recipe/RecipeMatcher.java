        ...
        public boolean match(int quantity, @Nullable RecipeMatcher.ItemCallback<T> itemCallback) {
            ...
            while (true) {
                ...
                if (intList == null) {
                    ...
                    for (int q = 0; q < this.totalIngredients; q++) {
                        for (int r = 0; r < this.totalRequiredItems; r++) {
                            if (this.isMissing(r, q)) {
                                this.markNotMissing(r, q);
                                RecipeMatcher.this.addInput(this.requiredItems.get(r), quantity);
                                if (bl2) {
                                    itemCallback.accept(this.requiredItems.get(r));
                                }
                                break;
                            }
                        }
        ...
        private IntList findIngredientItemLookup(int itemIndex) {
            ...
            while (!this.ingredientItemLookup.isEmpty()) {
                ...
                if (isItem(i - 1)) {
                    ...
                    int l = this.ingredientItemLookup.getInt(i - 1);
                    if (!this.getRequirement(l)) {
                        return this.ingredientItemLookup;
                    }

                    for (int m = 0; m < this.totalRequiredItems; m++) {
                        if (!this.isRequirementUnfulfilled(m) && this.isMissing(m, l)) {
                            assert this.matches(m, l);
                            this.markItemVisited(m);
                            this.ingredientItemLookup.add(m);
                            break;
                        }
                    }
                }

                int n = this.ingredientItemLookup.size();
                if (n == i) {
