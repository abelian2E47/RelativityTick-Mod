...
public record PotionContentsComponent(
    ...
    public void forEachEffect(Consumer<StatusEffectInstance> effectConsumer) {
        ...
        for (StatusEffectInstance statusEffectInstance2 : this.customEffects) {
            effectConsumer.accept(new StatusEffectInstance(statusEffectInstance2));
        }
    }
    ...
    public static void buildTooltip(Iterable<StatusEffectInstance> effects, Consumer<Text> textConsumer, float durationMultiplier, float tickRate) {
        ...
        for (StatusEffectInstance statusEffectInstance : effects) {
            ...
            registryEntry.value()
                .forEachAttributeModifier(statusEffectInstance.getAmplifier(), (attribute, modifier) -> list.add(new Pair<>(attribute, modifier)));
        ...
        if (!list.isEmpty()) {
            ...
            for (Pair<RegistryEntry<EntityAttribute>, EntityAttributeModifier> pair : list) {
                ...
                if (entityAttributeModifier.operation() != EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE
                    ...
                    f = entityAttributeModifier.value();
                } else {
                    f = entityAttributeModifier.value() * 100.0;
                }
                ...
                if (d > 0.0) {
                    textConsumer.accept(
                        Text.translatable(
                                ...
                                AttributeModifiersComponent.DECIMAL_FORMAT.format(f),
