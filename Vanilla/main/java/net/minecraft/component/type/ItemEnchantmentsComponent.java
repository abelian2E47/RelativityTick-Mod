...
public class ItemEnchantmentsComponent implements TooltipAppender {
    ...
    public static final Codec<ItemEnchantmentsComponent> CODEC = Codec.withAlternative(
        BASE_CODEC, INLINE_CODEC, map -> new ItemEnchantmentsComponent(map, true)
    );
        ...
        if (this.showInTooltip) {
            ...
            for (Entry<RegistryEntry<Enchantment>> entry : this.enchantments.object2IntEntrySet()) {
                ...
                if (!registryEntryList.contains(registryEntry2)) {
                    tooltip.accept(Enchantment.getName(entry.getKey(), entry.getIntValue()));
                }
            }
        }
    }
