...
public final class ComponentChanges {
    ...
    public static final Codec<ComponentChanges> CODEC = Codec.dispatchedMap(ComponentChanges.Type.CODEC, ComponentChanges.Type::getValueCodec)
        .xmap(changes -> {
            if (changes.isEmpty()) {
                return EMPTY;
            }
    ...
    public static final PacketCodec<RegistryByteBuf, ComponentChanges> PACKET_CODEC = new PacketCodec<RegistryByteBuf, ComponentChanges>() {
        public ComponentChanges decode(RegistryByteBuf registryByteBuf) {
            ...
            for (int m = 0; m < j; m++) {
                ComponentType<?> componentType2 = ComponentType.PACKET_CODEC.decode(registryByteBuf);
                reference2ObjectMap.put(componentType2, Optional.empty());
            }
        ...
        public void encode(RegistryByteBuf registryByteBuf, ComponentChanges componentChanges) {
            if (componentChanges.isEmpty()) {
                ...
                for (it.unimi.dsi.fastutil.objects.Reference2ObjectMap.Entry<ComponentType<?>, Optional<?>> entry2 : Reference2ObjectMaps.fastIterable(
                    componentChanges.changedComponents
                )) {
                    Optional<?> optional = entry2.getValue();
                    if (optional.isPresent()) {
                        ComponentType<?> componentType = entry2.getKey();
                        ComponentType.PACKET_CODEC.encode(registryByteBuf, componentType);
                        encode(registryByteBuf, componentType, optional.get());
                    }
                }

                for (it.unimi.dsi.fastutil.objects.Reference2ObjectMap.Entry<ComponentType<?>, Optional<?>> entry3 : Reference2ObjectMaps.fastIterable(
                    componentChanges.changedComponents
                )) {
                    if (entry3.getValue().isEmpty()) {
                        ComponentType<?> componentType2 = entry3.getKey();
    ...
    public ComponentChanges.AddedRemovedPair toAddedRemovedPair() {
        ...
        this.changedComponents.forEach((type, value) -> {
            if (value.isPresent()) {
                builder.put((ComponentType<?>)type, value.get());
            } else {
