...
public class MessageChain {
    ...
    public MessageChain.Unpacker getUnpacker(final PlayerPublicKey playerPublicKey) {
        final SignatureVerifier signatureVerifier = playerPublicKey.createSignatureInstance();
        return new MessageChain.Unpacker() {
            @Override
            public SignedMessage unpack(@Nullable MessageSignatureData signature, MessageBody body) throws MessageChain.MessageChainException {
                if (signature == null) {
                ...
                if (signedMessage.isExpiredOnServer(Instant.now())) {
                    MessageChain.LOGGER.warn("Received expired chat: '{}'. Is the client/server system time unsynchronized?", body.content());
                }
