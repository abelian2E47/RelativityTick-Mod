...
public final class PerlinNoiseSampler {
    ...
    public PerlinNoiseSampler(Random random) {
        ...
        for (int j = 0; j < 256; j++) {
            ...
            byte b = this.permutation[j];
            this.permutation[j] = this.permutation[j + k];
            this.permutation[j + k] = b;
        }
    }
    ...
    public double sample(double x, double y, double z, double yScale, double yMax) {
        ...
        if (yScale != 0.0) {
            ...
            o = MathHelper.floor(m / yScale + 1.0E-7F) * yScale;
        } else {
            o = 0.0;
        }
    ...
    private double sampleDerivative(int sectionX, int sectionY, int sectionZ, double localX, double localY, double localZ, double[] ds) {
        ...
        double ab = MathHelper.perlinFade(localY);
        double bb = MathHelper.perlinFade(localZ);
        double cb = MathHelper.lerp3(z, ab, bb, is[0], js[0], ks[0], ls[0], ms[0], ns[0], os[0], ps[0]);
        double db = MathHelper.lerp3(z, ab, bb, is[1], js[1], ks[1], ls[1], ms[1], ns[1], os[1], ps[1]);
        double eb = MathHelper.lerp3(z, ab, bb, is[2], js[2], ks[2], ls[2], ms[2], ns[2], os[2], ps[2]);
        double fb = MathHelper.lerp2(ab, bb, e - d, g - f, w - h, y - x);
        double gb = MathHelper.lerp2(bb, z, f - d, x - h, g - e, y - w);
        double hb = MathHelper.lerp2(z, ab, h - d, w - e, x - f, y - g);
        double ib = MathHelper.perlinFadeDerivative(localX);
        double jb = MathHelper.perlinFadeDerivative(localY);
        double kb = MathHelper.perlinFadeDerivative(localZ);
        double lb = cb + ib * fb;
        double mb = db + jb * gb;
        double nb = eb + kb * hb;
        ds[0] += lb;
        ds[1] += mb;
        ds[2] += nb;
        return MathHelper.lerp3(z, ab, bb, d, e, f, g, h, w, x, y);
    }
