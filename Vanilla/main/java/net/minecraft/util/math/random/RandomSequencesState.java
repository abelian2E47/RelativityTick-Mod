...
public class RandomSequencesState extends PersistentState {
    ...
    public static PersistentState.Type<RandomSequencesState> getPersistentStateType(long seed) {
        return new PersistentState.Type<>(
            () -> new RandomSequencesState(seed), (nbt, registries) -> fromNbt(seed, nbt), DataFixTypes.SAVED_DATA_RANDOM_SEQUENCES
        );
    }
    ...
    public NbtCompound writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        this.sequences
            .forEach((id, sequence) -> nbtCompound.put(id.toString(), RandomSequence.CODEC.encodeStart(NbtOps.INSTANCE, sequence).result().orElseThrow()));
    ...
    public static RandomSequencesState fromNbt(long seed, NbtCompound nbt) {
        ...
        for (String string : nbtCompound.getKeys()) {
            try {
                RandomSequence randomSequence = RandomSequence.CODEC.decode(NbtOps.INSTANCE, nbtCompound.get(string)).result().get().getFirst();
