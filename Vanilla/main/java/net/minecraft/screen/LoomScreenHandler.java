    ...
    private List<RegistryEntry<BannerPattern>> getPatternsFor(ItemStack stack) {
        if (stack.isEmpty()) {
            ...
            return stack.getItem() instanceof BannerPatternItem bannerPatternItem
                ? this.bannerPatternLookup.getOptional(bannerPatternItem.getPattern()).map(ImmutableList::copyOf).orElse(ImmutableList.of())
                : List.of();
        }
    }
    ...
    public void onContentChanged(Inventory inventory) {
        ...
        if (!itemStack.isEmpty() && !itemStack2.isEmpty()) {
            ...
            if (this.bannerPatterns.size() == 1) {
                ...
                RegistryEntry<BannerPattern> registryEntry3 = list.get(i);
                int j = this.bannerPatterns.indexOf(registryEntry3);
                if (j != -1) {
                    registryEntry = registryEntry3;
