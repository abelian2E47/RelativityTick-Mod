...
public abstract class ScreenHandler {
    ...
    private void internalOnSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
        ...
        if (actionType == SlotActionType.QUICK_CRAFT) {
            ...
            if (slotIndex == EMPTY_SPACE_SLOT_INDEX) {
                ...
                while (!itemStack4.isEmpty() && ItemStack.areItemsEqual(slot3.getStack(), itemStack4)) {
                ...
                ItemStack itemStack6 = this.getCursorStack();
                player.onPickupSlotClick(itemStack6, slot4.getStack(), clickType);
                if (!this.handleSlotClick(player, clickType, slot4, itemStack5, itemStack6)) {
                    if (itemStack5.isEmpty()) {
                        if (!itemStack6.isEmpty()) {
                            int o = clickType == ClickType.LEFT ? itemStack6.getCount() : 1;
                            this.setCursorStack(slot4.insertStack(itemStack6, o));
                        }
                    } else if (slot4.canTakeItems(player)) {
                        if (itemStack6.isEmpty()) {
                            int p = clickType == ClickType.LEFT ? itemStack5.getCount() : (itemStack5.getCount() + 1) / 2;
                            Optional<ItemStack> optional = slot4.tryTakeStackRange(p, Integer.MAX_VALUE, player);
                            optional.ifPresent(stack -> {
                                this.setCursorStack(stack);
                                slot4.onTakeItem(player, stack);
                            });
                        } else if (slot4.canInsert(itemStack6)) {
                            if (ItemStack.areItemsAndComponentsEqual(itemStack5, itemStack6)) {
                                int q = clickType == ClickType.LEFT ? itemStack6.getCount() : 1;
                                this.setCursorStack(slot4.insertStack(itemStack6, q));
                            } else if (itemStack6.getCount() <= slot4.getMaxItemCount(itemStack6)) {
                                this.setCursorStack(itemStack5);
                                slot4.setStack(itemStack6);
                            }
                        } else if (ItemStack.areItemsAndComponentsEqual(itemStack5, itemStack6)) {
                            Optional<ItemStack> optional2 = slot4.tryTakeStackRange(
                                itemStack5.getCount(), itemStack6.getMaxCount() - itemStack6.getCount(), player
                            );
                            optional2.ifPresent(stack -> {
                                itemStack6.increment(stack.getCount());
                                slot4.onTakeItem(player, stack);
                            });
                        }
                    }
            ...
            ItemStack itemStack7 = playerInventory.getStack(button);
            Slot slot5 = this.slots.get(slotIndex);
            ItemStack itemStack8 = slot5.getStack();
            if (!itemStack7.isEmpty() || !itemStack8.isEmpty()) {
                if (itemStack7.isEmpty()) {
                    if (slot5.canTakeItems(player)) {
                        playerInventory.setStack(button, itemStack8);
                        slot5.onTake(itemStack8.getCount());
                        slot5.setStack(ItemStack.EMPTY);
                        slot5.onTakeItem(player, itemStack8);
                    }
                } else if (itemStack8.isEmpty()) {
                    if (slot5.canInsert(itemStack7)) {
                        int r = slot5.getMaxItemCount(itemStack7);
                        if (itemStack7.getCount() > r) {
                            slot5.setStack(itemStack7.split(r));
                        } else {
                            playerInventory.setStack(button, ItemStack.EMPTY);
                            slot5.setStack(itemStack7);
                        }
                    }
                } else if (slot5.canTakeItems(player) && slot5.canInsert(itemStack7)) {
                    int s = slot5.getMaxItemCount(itemStack7);
                    if (itemStack7.getCount() > s) {
                        slot5.setStack(itemStack7.split(s));
                        slot5.onTakeItem(player, itemStack8);
            ...
            if (slot6.hasStack()) {
                ...
                this.setCursorStack(itemStack9.copyWithCount(itemStack9.getMaxCount()));
            }
        } else if (actionType == SlotActionType.THROW && this.getCursorStack().isEmpty() && slotIndex >= 0) {
            Slot slot7 = this.slots.get(slotIndex);
            int t = button == 0 ? 1 : slot7.getStack().getCount();
            if (!player.canDropItems()) {
                return;
            }

            ItemStack itemStack10 = slot7.takeStackRange(t, Integer.MAX_VALUE, player);
            player.dropItem(itemStack10, true);
            player.dropCreativeStack(itemStack10);
            if (button == 1) {
                while (!itemStack10.isEmpty() && ItemStack.areItemsEqual(slot7.getStack(), itemStack10)) {
                    if (!player.canDropItems()) {
                        return;
                    }

                    itemStack10 = slot7.takeStackRange(t, Integer.MAX_VALUE, player);
                    player.dropItem(itemStack10, true);
                    player.dropCreativeStack(itemStack10);
                }
            }
            ...
            ItemStack itemStack11 = this.getCursorStack();
            if (!itemStack11.isEmpty() && (!slot8.hasStack() || !slot8.canTakeItems(player))) {
                int u = button == 0 ? 0 : this.slots.size() - 1;
                int v = button == 0 ? 1 : -1;

                for (int w = 0; w < 2; w++) {
                    for (int x = u; x >= 0 && x < this.slots.size() && itemStack11.getCount() < itemStack11.getMaxCount(); x += v) {
                        Slot slot9 = this.slots.get(x);
                        if (slot9.hasStack()
                            && canInsertItemIntoSlot(slot9, itemStack11, true)
                            && slot9.canTakeItems(player)
                            && this.canInsertIntoSlot(itemStack11, slot9)) {
                            ItemStack itemStack12 = slot9.getStack();
                            if (w != 0 || itemStack12.getCount() != itemStack12.getMaxCount()) {
                                ItemStack itemStack13 = slot9.takeStackRange(itemStack12.getCount(), itemStack11.getMaxCount() - itemStack11.getCount(), player);
                                itemStack11.increment(itemStack13.getCount());
                            }
                        }
                    }
                }
        ...
        if (!stack.isEmpty()) {
            ...
            while (fromLast ? i >= startIndex : i < endIndex) {
                ...
                if (itemStack2.isEmpty() && slot2.canInsert(stack)) {
                    int l = slot2.getMaxItemCount(stack);
                    slot2.setStack(stack.split(Math.min(stack.getCount(), l)));
                    slot2.markDirty();
                    bl = true;
                    break;
                }
    ...
    public void copySharedSlots(ScreenHandler handler) {
        ...
        for (int j = 0; j < this.slots.size(); j++) {
            ...
            Integer integer = table.get(slot2.inventory, slot2.getIndex());
            if (integer != null) {
