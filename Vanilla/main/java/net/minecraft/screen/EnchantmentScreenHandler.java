                ...
                this.context
                    .run(
                        (world, pos) -> {
                            ...
                            for (int k = 0; k < 3; k++) {
                                if (this.enchantmentPower[k] > 0) {
                                    List<EnchantmentLevelEntry> list = this.generateEnchantments(
                                        world.getRegistryManager(), itemStack, k, this.enchantmentPower[k]
                                    );
                                    if (list != null && !list.isEmpty()) {
                                        EnchantmentLevelEntry enchantmentLevelEntry = list.get(this.random.nextInt(list.size()));
                                        this.enchantmentId[k] = indexedIterable.getRawId(enchantmentLevelEntry.enchantment);
                                        this.enchantmentLevel[k] = enchantmentLevelEntry.level;
                                    }
                                }
                            }
