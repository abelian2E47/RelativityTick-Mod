            ...
            itemStackTyped -> {
                ...
                if (optional2.filter(itemId -> itemId.equals("minecraft:white_banner")).isPresent()) {
                    return itemStackTyped.updateTyped(
                        opticFinder2, typed -> typed.update(DSL.remainderFinder(), ItemStackCustomNameToItemNameFix::fixOminousBanner)
                    );
                } else {
                    return optional2.filter(itemId -> itemId.equals("minecraft:filled_map")).isPresent()
                        ? itemStackTyped.updateTyped(
                            opticFinder2, typed -> typed.update(DSL.remainderFinder(), ItemStackCustomNameToItemNameFix::fixExplorerMaps)
                        )
                        : itemStackTyped;
                }
            }
