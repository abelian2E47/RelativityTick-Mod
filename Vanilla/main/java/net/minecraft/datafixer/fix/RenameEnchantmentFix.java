...
public class RenameEnchantmentFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped(
            this.name,
            type,
            itemStackTyped -> itemStackTyped.updateTyped(opticFinder, itemTagTyped -> itemTagTyped.update(DSL.remainderFinder(), this::fixIds))
        );
    }
                    ...
                    enchantments -> enchantments.map(
                        enchantmentDynamic -> enchantmentDynamic.update(
                            ...
                            idDynamic -> idDynamic.asString()
                                .map(
                                    oldId -> enchantmentDynamic.createString(this.oldToNewIds.getOrDefault(IdentifierNormalizingSchema.normalize(oldId), oldId))
                                )
