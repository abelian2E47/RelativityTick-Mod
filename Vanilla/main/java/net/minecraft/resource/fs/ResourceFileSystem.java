...
public class ResourceFileSystem extends FileSystem {
    ...
    private static ResourcePath toResourcePath(ResourceFileSystem.Directory root, ResourceFileSystem fileSystem, String name, @Nullable ResourcePath parent) {
        ...
        root.files
            .forEach(
                (fileName, path) -> object2ObjectOpenHashMap.put(fileName, new ResourcePath(fileSystem, fileName, resourcePath, new ResourceFile.File(path)))
            );
        root.children
            .forEach(
                (directoryName, directory) -> object2ObjectOpenHashMap.put(directoryName, toResourcePath(directory, fileSystem, directoryName, resourcePath))
            );
    ...
    public Path getPath(String first, String... more) {
        ...
        if (string.startsWith("/")) {
            ...
            for (String string3 : SEPARATOR_SPLITTER.split(string)) {
                if (string3.isEmpty()) {
                    throw new IllegalArgumentException("Empty paths not allowed");
                }

                resourcePath2 = new ResourcePath(this, string3, resourcePath2, ResourceFile.RELATIVE);
            }
