        ...
        public <T> int insert(List<T> items, T item, Function<T, ResourcePackPosition> profileGetter, boolean listInverted) {
            ...
            if (insertionPosition == BOTTOM) {
                ...
                for (j = items.size() - 1; j >= 0; j--) {
                    ResourcePackPosition resourcePackPosition2 = profileGetter.apply(items.get(j));
                    if (!resourcePackPosition2.fixedPosition() || resourcePackPosition2.defaultPosition() != this) {
                        break;
                    }
                }
