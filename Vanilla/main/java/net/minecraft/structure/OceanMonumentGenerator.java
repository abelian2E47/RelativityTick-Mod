...
public class OceanMonumentGenerator {
    ...
    public static class Base extends OceanMonumentGenerator.Piece {
        ...
        private List<OceanMonumentGenerator.PieceSetting> setupPieces(Random random) {
                    ...
                    for (int w = 0; w < 3; w++) {
                        ...
                        if (pieceSettings[x] != null) {
                            for (Direction direction : Direction.values()) {
                                ...
                                int ab = v + direction.getOffsetZ();
                                if (y >= 0 && y < 5 && ab >= 0 && ab < 5 && z >= 0 && z < 3) {
                                    int bb = getIndex(y, z, ab);
                                    if (pieceSettings[bb] != null) {
                                        if (ab == v) {
                                            pieceSettings[x].setNeighbor(direction, pieceSettings[bb]);
                                        } else {
                                            pieceSettings[x].setNeighbor(direction.getOpposite(), pieceSettings[bb]);
                                        }
                                    }
                                }
                            }
            ...
            for (OceanMonumentGenerator.PieceSetting pieceSetting5 : objectArrayList) {
                ...
                while (db < 2 && eb < 5) {
                    eb++;
                    int fb = random.nextInt(6);
                    if (pieceSetting5.neighborPresences[fb]) {
                        int gb = Direction.byId(fb).getOpposite().getId();
                        pieceSetting5.neighborPresences[fb] = false;
                        pieceSetting5.neighbors[fb].neighborPresences[gb] = false;
                        if (pieceSetting5.hasEntranceConnection(cb++) && pieceSetting5.neighbors[fb].hasEntranceConnection(cb++)) {
                            db++;
                        } else {
                            pieceSetting5.neighborPresences[fb] = true;
                            pieceSetting5.neighbors[fb].neighborPresences[gb] = true;
                        }
                    }
                }
            }
        ...
        public void generate(
            ...
            for (int p = 0; p < 5; p++) {
                this.setAirAndWater(world, chunkBox, -1 - p, 0 + p * 2, -1 - p, -1 - p, 23, 58 + p);
                this.setAirAndWater(world, chunkBox, 58 + p, 0 + p * 2, -1 - p, 58 + p, 23, 58 + p);
                this.setAirAndWater(world, chunkBox, 0 - p, 0 + p * 2, -1 - p, 57 + p, 23, -1 - p);
                this.setAirAndWater(world, chunkBox, 0 - p, 0 + p * 2, 58 + p, 57 + p, 23, 58 + p);
            }
        ...
        private void generateWing(boolean side, int start, StructureWorldAccess world, Random random, BlockBox box) {
            ...
            if (this.boxIntersects(box, start, 0, start + 23, 20)) {
                ...
                for (int o = 0; o < 4; o++) {
                    ...
                    this.addBlock(world, ALSO_PRISMARINE_BRICKS, p, 5, 5, box);
                }
        ...
        public void generate(
            ...
            for (int j = 1; j <= 6; j++) {
                ...
                for (int k = 0; k <= 15; k += 15) {
                    this.fillWithOutline(world, chunkBox, k, j, 0, k, j, 1, blockState, blockState, false);
                    this.fillWithOutline(world, chunkBox, k, j, 6, k, j, 9, blockState, blockState, false);
                    this.fillWithOutline(world, chunkBox, k, j, 14, k, j, 15, blockState, blockState, false);
                }

                this.fillWithOutline(world, chunkBox, 1, j, 0, 1, j, 0, blockState, blockState, false);
                this.fillWithOutline(world, chunkBox, 6, j, 0, 9, j, 0, blockState, blockState, false);
                this.fillWithOutline(world, chunkBox, 14, j, 0, 14, j, 0, blockState, blockState, false);
                this.fillWithOutline(world, chunkBox, 1, j, 15, 14, j, 15, blockState, blockState, false);
            }
            ...
            for (int l = 3; l <= 6; l += 3) {
                for (int m = 6; m <= 9; m += 3) {
                    this.addBlock(world, SEA_LANTERN, m, l, 6, chunkBox);
                    this.addBlock(world, SEA_LANTERN, m, l, 9, chunkBox);
                }
            }
    ...
    public static class DoubleYZRoom extends OceanMonumentGenerator.Piece {
        ...
        public void generate(
            ...
            for (int j = 1; j <= 7; j++) {
                ...
                this.fillWithOutline(world, chunkBox, 3, j, 7, 4, j, 8, blockState2, blockState2, false);
            }
    ...
    public static class Penthouse extends OceanMonumentGenerator.Piece {
        ...
        public void generate(
            ...
            for (int k = 0; k < 2; k++) {
                for (int l = 2; l <= 8; l += 3) {
                    this.fillWithOutline(world, chunkBox, j, 0, l, j, 2, l, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                }
        ...
        public void generate(
            ...
            if (this.roomType == 0) {
                ...
                for (int j = 18; j >= 7; j -= 3) {
                    this.addBlock(world, SEA_LANTERN, 6, 3, j, chunkBox);
                    this.addBlock(world, SEA_LANTERN, 16, 3, j, chunkBox);
                }
                ...
                for (int n = 0; n < 2; n++) {
                    this.addBlock(world, PRISMARINE_BRICKS, k, 6, 20, chunkBox);
                    this.addBlock(world, SEA_LANTERN, k, 5, 20, chunkBox);
                    this.addBlock(world, PRISMARINE_BRICKS, k, 4, 20, chunkBox);
                    k = 13;
                }
