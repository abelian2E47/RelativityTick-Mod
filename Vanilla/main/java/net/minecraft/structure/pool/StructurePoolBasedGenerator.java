...
public class StructurePoolBasedGenerator {
    ...
    public static Optional<Structure.StructurePosition> generate(
        ...
        PoolStructurePiece poolStructurePiece = new PoolStructurePiece(
            ...
            blockRotation,
            structurePoolElement.getBoundingBox(structureTemplateManager, blockPos3, blockRotation),
            liquidSettings
        );
                        ...
                        if (registryEntry2.value().getElementCount() == 0 && !registryEntry2.matchesKey(StructurePools.EMPTY)) {
                            StructurePoolBasedGenerator.LOGGER
                                .warn(
                                    ...
                                    registryEntry2.getKey().map(key -> key.getValue().toString()).orElse("<unregistered>")
                                );
                        } else {
                            ...
                            for (StructurePoolElement structurePoolElement2 : list) {
                                ...
                                for (BlockRotation blockRotation2 : BlockRotation.randomRotationOrder(this.random)) {
                                    ...
                                    if (modifyBoundingBox && blockBox2.getBlockCountY() <= 16) {
                                        n = list2.stream().mapToInt(jigsawInfo -> {
                                    ...
                                    for (StructureTemplate.JigsawBlockInfo jigsawBlockInfo2 : list2) {
                                        if (JigsawBlock.attachmentMatches(jigsawBlockInfo, jigsawBlockInfo2)) {
                                            ...
                                            int q = j - p + JigsawBlock.getFacing(structureBlockInfo.state()).getOffsetY();
                                            int r;
                                            if (bl && bl3) {
                                                r = i + q;
                                            } else {
                                                if (k == Integer.MIN_VALUE) {
                                                    k = this.chunkGenerator
                                                        .getHeightOnGround(
                                                            blockPos2.getX(), blockPos2.getZ(), Heightmap.Type.WORLD_SURFACE_WG, world, noiseConfig
                                                        );
                                                }
                                            ...
                                            int t = r - o;
                                            BlockBox blockBox4 = blockBox3.offset(0, t, 0);
                                            BlockPos blockPos6 = blockPos5.add(0, t, 0);
                                            if (n > 0) {
                                                int u = Math.max(n + 1, blockBox4.getMaxY() - blockBox4.getMinY());
                                                blockBox4.encompass(new BlockPos(blockBox4.getMinX(), blockBox4.getMinY() + u, blockBox4.getMinZ()));
                                            }

                                            if (!VoxelShapes.matchesAnywhere(
                                                mutableObject2.getValue(),
                                                VoxelShapes.cuboid(Box.from(blockBox4).contract(0.25)),
                                                BooleanBiFunction.ONLY_SECOND
                                            )) {
                                                mutableObject2.setValue(
                                                    VoxelShapes.combine(
                                                        mutableObject2.getValue(), VoxelShapes.cuboid(Box.from(blockBox4)), BooleanBiFunction.ONLY_FIRST
                                                    )
                                                );
