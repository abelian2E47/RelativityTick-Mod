...
public class MineshaftGenerator {
    ...
    private static MineshaftGenerator.MineshaftPart pickPiece(
        ...
        if (i >= 80) {
            ...
            BlockBox blockBox2 = MineshaftGenerator.MineshaftStairs.getBoundingBox(holder, random, x, y, z, orientation);
            if (blockBox2 != null) {
                return new MineshaftGenerator.MineshaftStairs(chainLength, blockBox2, orientation, type);
            }
        } else {
            BlockBox blockBox3 = MineshaftGenerator.MineshaftCorridor.getBoundingBox(holder, random, x, y, z, orientation);
            if (blockBox3 != null) {
    ...
    public static class MineshaftCorridor extends MineshaftGenerator.MineshaftPart {
        ...
        public static BlockBox getBoundingBox(StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation) {
            for (int i = random.nextInt(3) + 2; i > 0; i--) {
                ...
                if (holder.getIntersecting(blockBox4) == null) {
                    return blockBox4;
                }
            }
            ...
            if (i < 8) {
                if (direction != Direction.NORTH && direction != Direction.SOUTH) {
                    for (int m = this.boundingBox.getMinX() + 3; m + 3 <= this.boundingBox.getMaxX(); m += 5) {
                        ...
                        if (n == 0) {
                            MineshaftGenerator.pieceGenerator(
                                start, holder, random, m, this.boundingBox.getMinY(), this.boundingBox.getMinZ() - 1, Direction.NORTH, i + 1
                            );
                        } else if (n == 1) {
                            MineshaftGenerator.pieceGenerator(
                                start, holder, random, m, this.boundingBox.getMinY(), this.boundingBox.getMaxZ() + 1, Direction.SOUTH, i + 1
                            );
                        }
                    }
                } else {
        ...
        public void generate(
            ...
            if (!this.cannotGenerate(world, chunkBox)) {
                ...
                for (int r = 0; r <= 2; r++) {
                    for (int s = 0; s <= m; s++) {
                        this.tryPlaceFloor(world, chunkBox, blockState, r, -1, s);
                    }
                }
                ...
                if (this.length > 1) {
                    int u = m - 2;
                    this.fillSupportBeam(world, chunkBox, 0, -1, u);
                }
                ...
                if (this.hasRails) {
                    ...
                    for (int v = 0; v <= m; v++) {
                        BlockState blockState3 = this.getBlockAt(world, 1, -1, v, chunkBox);
                        if (!blockState3.isAir() && blockState3.isOpaqueFullCube()) {
                            float f = this.isUnderSeaLevel(world, 1, 0, v, chunkBox) ? 0.7F : 0.9F;
                            this.addBlockWithRandomThreshold(world, chunkBox, random, f, 1, 0, v, blockState2);
                        }
                    }
                }
            }
            ...
            if (box.contains(mutable)) {
                ...
                for (boolean bl2 = true; bl || bl2; j++) {
                    ...
                    if (bl2) {
                        ...
                        bl2 = j <= 50 && bl4 && mutable.getY() < world.getTopYInclusive();
                    }
                }
            }
        }
    ...
    public static class MineshaftCrossing extends MineshaftGenerator.MineshaftPart {
        ...
        public static BlockBox getBoundingBox(StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation) {
            ...
            blockBox4.move(x, y, z);
            return holder.getIntersecting(blockBox4) != null ? null : blockBox4;
        }
    ...
    abstract static class MineshaftPart extends StructurePiece {
        ...
        protected boolean cannotGenerate(WorldAccess world, BlockBox box) {
            ...
            for (int q = i; q <= l; q++) {
                for (int r = j; r <= m; r++) {
                    if (world.getBlockState(mutable.set(q, r, k)).isLiquid()) {
                        return true;
                    }

                    if (world.getBlockState(mutable.set(q, r, n)).isLiquid()) {
                        return true;
                    }
                }
            }
            ...
            for (int s = k; s <= n; s++) {
                for (int t = j; t <= m; t++) {
                    if (world.getBlockState(mutable.set(i, t, s)).isLiquid()) {
                        return true;
                    }

                    if (world.getBlockState(mutable.set(l, t, s)).isLiquid()) {
                        return true;
                    }
                }
            }
