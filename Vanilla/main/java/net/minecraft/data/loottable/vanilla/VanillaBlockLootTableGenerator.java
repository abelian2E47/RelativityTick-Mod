    ...
    protected void generate() {
                                        ...
                                        SetCountLootFunction.builder(ConstantLootNumberProvider.create(3.0F))
                                            .conditionally(
                                                BlockStatePropertyLootCondition.builder(block)
                                                    .properties(StatePredicate.Builder.create().exactMatch(CocoaBlock.AGE, 2))
                                            )
                                    )
                            )
                        )
            ...
            block -> LootTable.builder()
                ...
                .pool(
                    LootPool.builder()
                        ...
                        .conditionally(
                            BlockStatePropertyLootCondition.builder(block).properties(StatePredicate.Builder.create().exactMatch(ComposterBlock.LEVEL, 8))
                        )
                )
        );
        ...
        this.addDrop(
            ...
            this.applyExplosionDecay(
                Blocks.MANGROVE_PROPAGULE,
                LootTable.builder().pool(LootPool.builder().conditionally(builder4).with(ItemEntry.builder(Items.MANGROVE_PROPAGULE)))
            )
        );
        this.addDrop(
        ...
        this.addDrop(
            ...
            block -> this.dropsWithSilkTouchOrShears(
                block, (LootPoolEntry.Builder<?>)this.addSurvivesExplosionCondition(block, ItemEntry.builder(Items.STRING))
            )
        );
        this.addDrop(
            Blocks.DEAD_BUSH,
