...
public class ParticleUtil {
    ...
    public static void spawnSmashAttackParticles(WorldAccess world, BlockPos pos, int count) {
        ...
        for (int k = 0; k < count / 1.5F; k++) {
            double l = vec3d.x + 3.5 * Math.cos(k) + world.getRandom().nextGaussian() / 2.0;
            double m = vec3d.y;
            double n = vec3d.z + 3.5 * Math.sin(k) + world.getRandom().nextGaussian() / 2.0;
            ...
            world.addParticle(blockStateParticleEffect, l, m, n, o, p, q);
        }
    }
}
