        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            ...
            for (int l = 0; l <= k; l++) {
                if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)
                    ...
                    if (l == k) {
                        structureWorldAccess.setBlockState(blockPos2, blockState.with(KelpBlock.AGE, random.nextInt(4) + 20), Block.NOTIFY_LISTENERS);
                        i++;
                    } else {
                    ...
                    if (blockState.canPlaceAt(structureWorldAccess, blockPos3) && !structureWorldAccess.getBlockState(blockPos3.down()).isOf(Blocks.KELP)) {
                        structureWorldAccess.setBlockState(blockPos3, blockState.with(KelpBlock.AGE, random.nextInt(4) + 20), Block.NOTIFY_LISTENERS);
                        i++;
                    }
                    break;
                }
