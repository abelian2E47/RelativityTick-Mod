    ...
    public static <T> List<PlacedFeatureIndexer.IndexedFeatures> collectIndexedFeatures(
        ...
        for (T object : biomes) {
            ...
            for (int k = 0; k < list.size(); k++) {
                Set<IndexedFeature> set = map.computeIfAbsent(list.get(k), feature -> new TreeSet<>(comparator));
                if (k < list.size() - 1) {
        ...
        List<IndexedFeature> list3 = Lists.newArrayList();
        ...
        for (IndexedFeature indexedFeature : map.keySet()) {
            ...
            if (!set2.contains(indexedFeature) && TopologicalSorts.sort(map, set2, set3, list3::add, indexedFeature)) {
        ...
        for (int m = 0; m < i; m++) {
            int n = m;
            List<PlacedFeature> list5 = list3.stream().filter(feature -> feature.step() == n).map(IndexedFeature::feature).collect(Collectors.toList());
            builder.add(new PlacedFeatureIndexer.IndexedFeatures(list5));
        }
