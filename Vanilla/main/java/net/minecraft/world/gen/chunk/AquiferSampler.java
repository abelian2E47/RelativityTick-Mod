    ...
    class Impl implements AquiferSampler {
        ...
        public BlockState apply(DensityFunction.NoisePos pos, double density) {
            ...
            for (int w = 0; w <= 1; w++) {
                for (int x = -1; x <= 1; x++) {
                    for (int y = 0; y <= 1; y++) {
                        ...
                        int ab = m + x;
                        int bb = n + y;
                        int cb = this.index(z, ab, bb);
                        long db = this.blockPositions[cb];
                        long eb;
                        if (db != Long.MAX_VALUE) {
                            eb = db;
                        } else {
                            Random random = this.randomDeriver.split(z, ab, bb);
                            eb = BlockPos.asLong(z * 16 + random.nextInt(10), ab * 12 + random.nextInt(9), bb * 16 + random.nextInt(10));
                            this.blockPositions[cb] = eb;
                        }

                        int gb = BlockPos.unpackLongX(eb) - i;
                        int hb = BlockPos.unpackLongY(eb) - j;
                        int ib = BlockPos.unpackLongZ(eb) - k;
                        int jb = gb * gb + hb * hb + ib * ib;
                        if (o >= jb) {
                            v = u;
                            u = t;
                            t = s;
            ...
            if (d <= 0.0) {
                ...
                if (h > 0.0) {
                    double kb = d * h * this.calculateDensity(pos, mutableDouble, fluidLevel4, fluidLevel5);
                    if (density + kb > 0.0) {
                        this.needsFluidTick = false;
                        return null;
                    }
                ...
                boolean bl2 = h >= NEEDS_FLUID_TICK_DISTANCE_THRESHOLD && !fluidLevel4.equals(fluidLevel5);
        ...
        private double calculateDensity(
            ...
            if ((!blockState.isOf(Blocks.LAVA) || !blockState2.isOf(Blocks.WATER)) && (!blockState.isOf(Blocks.WATER) || !blockState2.isOf(Blocks.LAVA))) {
                ...
                if (!(q < -2.0) && !(q > 2.0)) {
                    ...
                    if (Double.isNaN(x)) {
                        double y = this.barrierNoise.sample(pos);
        ...
        private int getFluidBlockY(int blockX, int blockY, int blockZ, AquiferSampler.FluidLevel defaultFluidLevel, int surfaceHeightEstimate, boolean bl) {
            ...
            int n;
            if (e > 0.0) {
                n = defaultFluidLevel.y;
            } else if (d > 0.0) {
                n = this.getNoiseBasedFluidLevel(blockX, blockY, blockZ, surfaceHeightEstimate);
            } else {
                n = DimensionType.field_35479;
            }
