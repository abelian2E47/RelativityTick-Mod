                    ...
                    for (int s = l - 1; s >= 0; s--) {
                        ...
                        for (int w = 0; w < k; w++) {
                            ...
                            for (int z = 0; z < k; z++) {
                                ...
                                int bb = ab & 15;
                                double f = (double)z / k;
                                chunkNoiseSampler.interpolateZ(ab, f);
                                BlockState blockState = chunkNoiseSampler.sampleBlockState();
                                if (blockState == null) {
                                    blockState = this.settings.value().defaultBlock();
                                }

                                blockState = this.getBlockState(chunkNoiseSampler, x, t, ab, blockState);
                                if (blockState != AIR && !SharedConstants.isOutsideGenerationArea(chunk.getPos())) {
                                    chunkSection.setBlockState(y, u, bb, blockState, false);
                                    heightmap.trackUpdate(y, t, bb, blockState);
                                    heightmap2.trackUpdate(y, t, bb, blockState);
