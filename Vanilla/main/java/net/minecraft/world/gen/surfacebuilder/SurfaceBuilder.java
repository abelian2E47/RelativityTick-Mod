...
public class SurfaceBuilder {
            ...
            for (int l = 0; l < 16; l++) {
                ...
                for (int u = p; u >= t; u--) {
                    ...
                    if (blockState.isAir()) {
                        ...
                        q++;
                        int w = u - s + 1;
                        materialRuleContext.initVerticalContext(q, w, r, m, u, n);
                        if (blockState == this.defaultState) {
                            BlockState blockState3 = blockStateRule.tryApply(m, u, n);
                            if (blockState3 != null) {
                                blockColumn.setState(u, blockState3);
                            }
                        }
                    }
                }
    ...
    private void placeBadlandsPillar(BlockColumn column, int x, int z, int surfaceY, HeightLimitView chunk) {
        ...
        if (!(e <= 0.0)) {
            ...
            if (surfaceY <= j) {
                ...
                for (int l = j; l >= chunk.getBottomY() && column.getState(l).isAir(); l--) {
                    column.setState(l, this.defaultState);
                }
            }
        }
    }
    ...
    private void placeIceberg(int minY, Biome biome, BlockColumn column, BlockPos.Mutable mutablePos, int x, int z, int surfaceY) {
        ...
        if (!(e <= 1.8)) {
            ...
            int n = this.seaLevel + 18 + random.nextInt(10);
            int o = 0;

            for (int p = Math.max(surfaceY, (int)l + 1); p >= minY; p--) {
                if (column.getState(p).isAir() && p < (int)l && random.nextDouble() > 0.01
                    || column.getState(p).isOf(Blocks.WATER) && p > (int)j && p < this.seaLevel && j != 0.0 && random.nextDouble() > 0.15) {
                    if (o <= m && p > n) {
                        column.setState(p, SNOW_BLOCK);
                        o++;
                    } else {
                        column.setState(p, PACKED_ICE);
                    }
                }
            }
        }
