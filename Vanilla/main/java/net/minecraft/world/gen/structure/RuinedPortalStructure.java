    ...
    private static int getFloorHeight(
        ...
        for (s = j; s > i; s--) {
            ...
            for (VerticalBlockSample verticalBlockSample : list2) {
                BlockState blockState = verticalBlockSample.getState(s);
                if (type.getBlockPredicate().test(blockState) && ++t == 3) {
                    return s;
                }
            }
        }
