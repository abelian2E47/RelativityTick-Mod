...
public class NetherPortal {
    ...
    public static Vec3d entityPosInPortal(BlockLocating.Rectangle portalRect, Direction.Axis portalAxis, Vec3d entityPos, EntityDimensions entityDimensions) {
        ...
        if (e > 0.0) {
            ...
            i = MathHelper.clamp(MathHelper.getLerpProgress(entityPos.getComponentAlongAxis(axis) - blockPos.getComponentAlongAxis(axis), 0.0, e), 0.0, 1.0);
        } else {
            i = 0.0;
        }

        Direction.Axis axis2 = portalAxis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
        double k = entityPos.getComponentAlongAxis(axis2) - (blockPos.getComponentAlongAxis(axis2) + 0.5);
