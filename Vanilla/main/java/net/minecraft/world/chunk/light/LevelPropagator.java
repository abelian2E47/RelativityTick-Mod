...
public abstract class LevelPropagator {
    ...
    private void updateLevel(long sourceId, long id, int level, int currentLevel, int i, boolean decrease) {
        if (!this.isMarker(id)) {
            ...
            int l = this.calculateLevel(currentLevel, i);
            if (currentLevel != j) {
                int m = this.calculateLevel(currentLevel, j);
                if (l != m && !bl) {
                    this.pendingUpdateQueue.remove(id, l, m);
                }

                this.pendingUpdateQueue.enqueue(id, m);
                this.pendingUpdates.put(id, (byte)j);
            } else if (!bl) {
                this.pendingUpdateQueue.remove(id, l, this.levelCount);
