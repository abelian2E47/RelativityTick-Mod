...
public abstract class LightStorage<M extends ChunkToNibbleArrayMap<M>> {
    ...
    protected void updateLight(ChunkLightProvider<M, ?> lightProvider) {
        if (this.hasLightUpdates) {
            ...
            for (long l : this.sectionsToRemove) {
            ...
            for (long m : this.sectionsToRemove) {
                this.onUnloadSection(m);
                this.dirtySections.add(m);
            }

            this.sectionsToRemove.clear();
            ObjectIterator<Entry<ChunkNibbleArray>> objectIterator = Long2ObjectMaps.fastIterator(this.queuedSections);

            while (objectIterator.hasNext()) {
                Entry<ChunkNibbleArray> entry = objectIterator.next();
                ...
                for (int k = -1; k <= 1; k++) {
                    for (int l = -1; l <= 1; l++) {
                        if (j != 0 || k != 0 || l != 0) {
                            ...
                            this.setSectionPropagation(
                                m, LightStorage.PropagationFlags.withNeighborCount(d, LightStorage.PropagationFlags.getNeighborCount(d) + i)
                            );
                        }
                    }
                }
