...
public class UpgradeData {
    ...
    private void upgradeCenter(WorldChunk chunk) {
        ...
        for (int p = 0; p < this.centerIndicesToUpgrade.length; p++) {
            if (this.centerIndicesToUpgrade[p] != null) {
                LOGGER.warn("Discarding update data for section {} for chunk ({} {})", worldAccess.sectionIndexToCoord(p), chunkPos.x, chunkPos.z);
            }

            this.centerIndicesToUpgrade[p] = null;
        }
    }
    ...
    public NbtCompound toNbt() {
        ...
        if (!this.fluidTicks.isEmpty()) {
            ...
            this.fluidTicks.forEach(fluidTick -> nbtList2.add(fluidTick.toNbt(fluid -> Registries.FLUID.getId(fluid).toString())));
            ...
            public BlockState getUpdatedState(
                ...
                if (otherState.isOf(oldState.getBlock())
                    ...
                    if (direction.getAxis() != direction2.getAxis() && direction2 == otherState.get(ChestBlock.FACING)) {
                        ...
                        world.setBlockState(
                            otherPos, otherState.with(ChestBlock.CHEST_TYPE, chestType.getOpposite()), Block.NOTIFY_LISTENERS | Block.FORCE_STATE
                        );
