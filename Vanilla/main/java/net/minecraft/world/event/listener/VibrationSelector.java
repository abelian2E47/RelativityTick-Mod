...
public class VibrationSelector {
    public static final Codec<VibrationSelector> CODEC = RecordCodecBuilder.create(
        instance -> instance.group(
                ...
                Codec.LONG.fieldOf("tick").forGetter(vibrationSelector -> vibrationSelector.current.map(Pair::getRight).orElse(-1L))
            )
    ...
    public Optional<Vibration> getVibrationToTick(long currentTick) {
        if (this.current.isEmpty()) {
            ...
            return this.current.get().getRight() < currentTick ? Optional.of(this.current.get().getLeft()) : Optional.empty();
        }
    }
