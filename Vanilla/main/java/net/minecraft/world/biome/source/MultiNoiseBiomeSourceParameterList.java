...
public class MultiNoiseBiomeSourceParameterList {
    ...
    public static Map<MultiNoiseBiomeSourceParameterList.Preset, MultiNoiseUtil.Entries<RegistryKey<Biome>>> getPresetToEntriesMap() {
        return MultiNoiseBiomeSourceParameterList.Preset.BY_IDENTIFIER
            ...
            .collect(
                Collectors.toMap(
                    preset -> (MultiNoiseBiomeSourceParameterList.Preset)preset, preset -> preset.biomeSourceFunction().apply(biomeKey -> biomeKey)
                )
            );
    }
    ...
    public record Preset(Identifier id, MultiNoiseBiomeSourceParameterList.Preset.BiomeSourceFunction biomeSourceFunction) {
            ...
            new MultiNoiseBiomeSourceParameterList.Preset.BiomeSourceFunction() {
                ...
                public <T> MultiNoiseUtil.Entries<T> apply(Function<RegistryKey<Biome>, T> biomeEntryGetter) {
                    return new MultiNoiseUtil.Entries<>(
                        List.of(
                            Pair.of(
                                MultiNoiseUtil.createNoiseHypercube(0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F), biomeEntryGetter.apply(BiomeKeys.NETHER_WASTES)
                            ),
                            Pair.of(
                                MultiNoiseUtil.createNoiseHypercube(0.0F, -0.5F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F),
                                biomeEntryGetter.apply(BiomeKeys.SOUL_SAND_VALLEY)
                            ),
                            Pair.of(
                                MultiNoiseUtil.createNoiseHypercube(0.4F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F), biomeEntryGetter.apply(BiomeKeys.CRIMSON_FOREST)
                            ),
                            Pair.of(
                                MultiNoiseUtil.createNoiseHypercube(0.0F, 0.5F, 0.0F, 0.0F, 0.0F, 0.0F, 0.375F),
                                biomeEntryGetter.apply(BiomeKeys.WARPED_FOREST)
                            ),
                            Pair.of(
                                MultiNoiseUtil.createNoiseHypercube(-0.5F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.175F),
                                biomeEntryGetter.apply(BiomeKeys.BASALT_DELTAS)
                            )
                        )
                    );
                }
        ...
        public static final MultiNoiseBiomeSourceParameterList.Preset OVERWORLD = new MultiNoiseBiomeSourceParameterList.Preset(
            Identifier.ofVanilla("overworld"), new MultiNoiseBiomeSourceParameterList.Preset.BiomeSourceFunction() {
                @Override
                public <T> MultiNoiseUtil.Entries<T> apply(Function<RegistryKey<Biome>, T> biomeEntryGetter) {
                    return MultiNoiseBiomeSourceParameterList.Preset.getOverworldEntries(biomeEntryGetter);
                }
            }
        );
        ...
        public static final Codec<MultiNoiseBiomeSourceParameterList.Preset> CODEC = Identifier.CODEC
            .flatXmap(
                id -> Optional.ofNullable(BY_IDENTIFIER.get(id)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown preset: " + id)),
