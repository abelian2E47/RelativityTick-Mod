...
public class GameRules {
    ...
    public static final GameRules.Key<GameRules.IntRule> RANDOM_TICK_SPEED = register(
        "randomTickSpeed", GameRules.Category.UPDATES, GameRules.IntRule.create(3)
    );
                    ...
                    serverPlayerEntity.networkHandler
                        .sendPacket(
                            new GameStateChangeS2CPacket(
                                GameStateChangeS2CPacket.LIMITED_CRAFTING_TOGGLED, rule.get() ? 1.0F : GameStateChangeS2CPacket.DEMO_OPEN_SCREEN
                            )
                        );
                }
            }
            ...
            (server, rule) -> {
                for (ServerPlayerEntity serverPlayerEntity : server.getPlayerManager().getPlayerList()) {
                    serverPlayerEntity.networkHandler
                        .sendPacket(
                            new GameStateChangeS2CPacket(
                                GameStateChangeS2CPacket.IMMEDIATE_RESPAWN, rule.get() ? 1.0F : GameStateChangeS2CPacket.DEMO_OPEN_SCREEN
                            )
                        );
                }
            }
    ...
    public GameRules(FeatureSet enabledFeatures) {
        this(streamAllRules(enabledFeatures).collect(ImmutableMap.toImmutableMap(Entry::getKey, entry -> entry.getValue().createRule())), enabledFeatures);
    }
