...
public class SteppedCommandAction<T, P> implements CommandAction<T> {
    ...
    public void execute(CommandExecutionContext<T> context, Frame frame) {
        ...
        context.enqueueCommand(this.wrapper.create(frame, object));
        if (++this.nextActionIndex < this.actions.size()) {
            context.enqueueCommand(this.selfCommandQueueEntry);
        }
    }

    public static <T, P> void enqueueCommands(
        CommandExecutionContext<T> context, Frame frame, List<P> actions, SteppedCommandAction.ActionWrapper<T, P> wrapper
    ) {
        int i = actions.size();
        switch (i) {
            case 0:
