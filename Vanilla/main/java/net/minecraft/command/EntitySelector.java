...
public class EntitySelector {
    ...
    public List<? extends Entity> getEntities(ServerCommandSource source) throws CommandSyntaxException {
        ...
        if (this.uuid != null) {
            ...
            Predicate<Entity> predicate2 = this.getPositionPredicate(vec3d, box, source.getEnabledFeatures());
            List<Entity> list = new ObjectArrayList<>();
            if (this.isLocalWorldOnly()) {
                this.appendEntitiesFromWorld(list, source.getWorld(), box, predicate2);
            } else {
                for (ServerWorld serverWorld2 : source.getServer().getWorlds()) {
                    this.appendEntitiesFromWorld(list, serverWorld2, box, predicate2);
                }
            }
    ...
    public List<ServerPlayerEntity> getPlayers(ServerCommandSource source) throws CommandSyntaxException {
        ...
        if (this.uuid != null) {
            ServerPlayerEntity serverPlayerEntity2 = source.getServer().getPlayerManager().getPlayer(this.uuid);
            return serverPlayerEntity2 == null ? List.of() : List.of(serverPlayerEntity2);
        }
        ...
        Predicate<Entity> predicate = this.getPositionPredicate(vec3d, box, null);
        if (this.senderOnly) {
            return source.getEntity() instanceof ServerPlayerEntity serverPlayerEntity3 && predicate.test(serverPlayerEntity3)
                ? List.of(serverPlayerEntity3)
                : List.of();
        }
        ...
        if (this.isLocalWorldOnly()) {
            ...
            for (ServerPlayerEntity serverPlayerEntity4 : source.getServer().getPlayerManager().getPlayerList()) {
