                ...
                new TestListener() {
                    private void onFinished() {
                        if (testSet.isDone()) {
                            ...
                            longSet.forEach(
                                chunkPos -> TestRunContext.this.world.setChunkForced(ChunkPos.getPackedX(chunkPos), ChunkPos.getPackedZ(chunkPos), false)
                            );
    ...
    private void onFinish() {
        if (!this.toBeRetried.isEmpty()) {
            LOGGER.info(
                ...
                this.toBeRetried.stream().map(state -> state.getTestFunction().templatePath()).collect(Collectors.joining(", "))
            );
