...
public interface Registry<T> extends Keyable, RegistryWrapper.Impl<T>, IndexedIterable<T> {
    ...
    default Codec<RegistryEntry<T>> getEntryCodec() {
        return this.getReferenceEntryCodec().flatComapMap(entry -> (RegistryEntry<T>)entry, this::validateReference);
    }
    ...
    default Iterable<RegistryEntry<T>> iterateEntries(TagKey<T> tag) {
        return DataFixUtils.orElse(this.getOptional(tag), List.<T>of());
    }
