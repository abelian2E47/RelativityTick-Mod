        ...
        if (ops instanceof RegistryOps<T> registryOps) {
            ...
            if (optional.isPresent()) {
                ...
                return this.entryListStorageCodec
                    ...
                    .flatMap(
                        pair -> {
                            DataResult<RegistryEntryList<E>> dataResult = pair.getFirst()
                                .map(
                                    tag -> get(registryEntryLookup, (TagKey<E>)tag),
                                    entries -> DataResult.success(RegistryEntryList.of((List<? extends RegistryEntry<E>>)entries))
                                );
                            return dataResult.map(entries -> Pair.of((RegistryEntryList<E>)entries, (T)pair.getSecond()));
                        }
                    );
            }
        }
