    ...
    private void clientTick() {
        ...
        if (!bl || !this.random.nextBoolean()) {
            ...
            for (int k = 0; k < i; k++) {
                ...
                double d = this.getX() + MathHelper.cos(l) * m;
                double e = this.getY();
                double n = this.getZ() + MathHelper.sin(l) * m;
                if (particleEffect.getType() == ParticleTypes.ENTITY_EFFECT) {
                    if (bl && this.random.nextBoolean()) {
                        this.getWorld().addImportantParticle(EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, -1), d, e, n, 0.0, 0.0, 0.0);
                    } else {
                        this.getWorld().addImportantParticle(particleEffect, d, e, n, 0.0, 0.0, 0.0);
                    }
                } else if (bl) {
                    this.getWorld().addImportantParticle(particleEffect, d, e, n, 0.0, 0.0, 0.0);
                } else {
                    this.getWorld()
                        .addImportantParticle(particleEffect, d, e, n, (0.5 - this.random.nextDouble()) * 0.15, 0.01F, (0.5 - this.random.nextDouble()) * 0.15);
                }
            }
        }
    }
