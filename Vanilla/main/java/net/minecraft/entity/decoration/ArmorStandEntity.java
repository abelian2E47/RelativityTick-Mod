...
public class ArmorStandEntity extends LivingEntity {
    ...
    public static final TrackedData<EulerAngle> TRACKER_LEFT_ARM_ROTATION = DataTracker.registerData(
    ...
    public static final TrackedData<EulerAngle> TRACKER_LEFT_LEG_ROTATION = DataTracker.registerData(
        ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION
    );
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("HandItems", NbtElement.LIST_TYPE)) {
            NbtList nbtList2 = nbt.getList("HandItems", NbtElement.COMPOUND_TYPE);

            for (int j = 0; j < this.heldItems.size(); j++) {
                NbtCompound nbtCompound2 = nbtList2.getCompound(j);
                this.heldItems.set(j, ItemStack.fromNbtOrEmpty(this.getRegistryManager(), nbtCompound2));
            }
        }
        ...
        NbtCompound nbtCompound3 = nbt.getCompound("Pose");
        this.readPoseNbt(nbtCompound3);
    }
    ...
    public void handleStatus(byte status) {
        if (status == EntityStatuses.HIT_ARMOR_STAND) {
            if (this.getWorld().isClient) {
                this.getWorld()
                    .playSound(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ARMOR_STAND_HIT, this.getSoundCategory(), 0.3F, 1.0F, false);
    ...
    private void onBreak(ServerWorld world, DamageSource damageSource) {
        ...
        for (int j = 0; j < this.armorItems.size(); j++) {
            ...
            if (!itemStack2.isEmpty()) {
                Block.dropStack(this.getWorld(), this.getBlockPos().up(), itemStack2);
