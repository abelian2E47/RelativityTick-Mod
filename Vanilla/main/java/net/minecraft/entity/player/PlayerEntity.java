...
public abstract class PlayerEntity extends LivingEntity {
    ...
    protected static final TrackedData<NbtCompound> RIGHT_SHOULDER_ENTITY = DataTracker.registerData(
        PlayerEntity.class, TrackedDataHandlerRegistry.NBT_COMPOUND
    );
    ...
    public void tickMovement() {
        ...
        if (!this.getWorld().isClient && (this.fallDistance > 0.5F || this.isTouchingWater())
            ...
            if (!target.handleAttack(this)) {
                ...
                if (target.getType().isIn(EntityTypeTags.REDIRECTABLE_PROJECTILE)
                    ...
                    if (f > 0.0F || g > 0.0F) {
                        ...
                        boolean bl6 = target.sidedDamage(damageSource, i);
                        if (bl6) {
                            ...
                            if (bl4) {
                                this.getWorld()
                                    .playSound(
                                        null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, this.getSoundCategory(), 1.0F, 1.0F
                                    );
                            ...
                            if (this.getWorld() instanceof ServerWorld serverWorld2) {
                                if (entity instanceof LivingEntity livingEntity4) {
                                    bl7 = itemStack.postHit(livingEntity4, this);
                                }
                            ...
                            if (target instanceof LivingEntity) {
                                ...
                                if (this.getWorld() instanceof ServerWorld && n > 2.0F) {
                                    ...
                                    ((ServerWorld)this.getWorld())
                                        .spawnParticles(
                                            ParticleTypes.DAMAGE_INDICATOR, target.getX(), target.getBodyY(0.5), target.getZ(), o, 0.1, 0.0, 0.1, 0.2
                                        );
                                }
                            }
                            ...
                            this.getWorld()
                                .playSound(
                                    null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_NODAMAGE, this.getSoundCategory(), 1.0F, 1.0F
                                );
                        }
                    }
                }
    ...
    public void travel(Vec3d movementInput) {
        if (this.hasVehicle()) {
            ...
            if (this.getAbilities().flying) {
                ...
                super.travel(movementInput);
                this.setVelocity(this.getVelocity().withAxis(Direction.Axis.Y, f * 0.6));
            } else {
                super.travel(movementInput);
            }
        }
    ...
    public Vec3d getLeashPos(float delta) {
        ...
        if (this.isGliding() || this.isUsingRiptide()) {
            ...
            double n = this.isInSneakingPose() ? -0.2 : 0.07;
            return this.getLerpedPos(delta).add(new Vec3d(d, m, n).rotateY(-g));
        }
    }
