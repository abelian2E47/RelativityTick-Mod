...
public class EndermanEntity extends HostileEntity implements Angerable {
    ...
    public void playAngrySound() {
        if (this.age >= this.lastAngrySoundAge + 400) {
            ...
            if (!this.isSilent()) {
                this.getWorld()
                    .playSound(this.getX(), this.getEyeY(), this.getZ(), SoundEvents.ENTITY_ENDERMAN_STARE, this.getSoundCategory(), 2.5F, 1.0F, false);
            }
        }
    }
    ...
    private boolean teleportTo(double x, double y, double z) {
        ...
        if (bl && !bl2) {
            ...
            if (bl3) {
                ...
                if (!this.isSilent()) {
                    this.getWorld()
                        .playSound(null, this.prevX, this.prevY, this.prevZ, SoundEvents.ENTITY_ENDERMAN_TELEPORT, this.getSoundCategory(), 1.0F, 1.0F);
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (!source.isIn(DamageTypeTags.IS_PROJECTILE) && !bl) {
            boolean bl3 = super.damage(world, source, amount);
