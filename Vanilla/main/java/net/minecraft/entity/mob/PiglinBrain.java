...
public class PiglinBrain {
    ...
    protected static void consumeOffHandItem(ServerWorld world, PiglinEntity piglin, boolean barter) {
        ...
        if (piglin.isAdult()) {
            ...
            boolean bl3 = !piglin.tryEquip(world, itemStack).isEmpty();
            if (!bl3) {
    ...
    private static Optional<? extends LivingEntity> getPreferredTarget(ServerWorld world, PiglinEntity piglin) {
        ...
        Optional<MobEntity> optional3 = brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_NEMESIS);
        if (optional3.isPresent()) {
            return optional3;
        }

        Optional<PlayerEntity> optional4 = brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GOLD);
        return optional4.isPresent() && Sensor.testAttackableTargetPredicate(world, piglin, optional4.get()) ? optional4 : Optional.empty();
    }
