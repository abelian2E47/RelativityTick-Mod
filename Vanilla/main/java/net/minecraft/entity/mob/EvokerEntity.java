    ...
    class ConjureFangsGoal extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        protected void castSpell() {
            ...
            if (EvokerEntity.this.squaredDistanceTo(livingEntity) < 9.0) {
                ...
                for (int j = 0; j < 8; j++) {
                    float h = f + j * (float) Math.PI * 2.0F / 8.0F + (float) (Math.PI * 2.0 / 5.0);
                    this.conjureFangs(EvokerEntity.this.getX() + MathHelper.cos(h) * 2.5, EvokerEntity.this.getZ() + MathHelper.sin(h) * 2.5, d, e, h, 3);
                }
            } else {
                for (int k = 0; k < 16; k++) {
                    double l = 1.25 * (k + 1);
                    int m = 1 * k;
                    this.conjureFangs(EvokerEntity.this.getX() + MathHelper.cos(f) * l, EvokerEntity.this.getZ() + MathHelper.sin(f) * l, d, e, f, m);
                }
            }
        }
