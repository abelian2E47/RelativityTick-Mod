...
public class CamelEntity extends AbstractHorseEntity {
    ...
    private double getPassengerAttachmentY(boolean primaryPassenger, float tickDelta, EntityDimensions dimensions, float scaleFactor) {
        ...
        if (bl) {
            ...
            float n = MathHelper.clamp((float)this.getLastPoseTickDelta() + tickDelta, 0.0F, i);
            boolean bl3 = n < j;
            float o = bl3 ? n / j : (n - j) / (i - j);
            float p = f - k * g;
            d += bl2 ? MathHelper.lerp(o, bl3 ? f : p, bl3 ? p : h) : MathHelper.lerp(o, bl3 ? h - f : h - p, bl3 ? h - p : 0.0F);
        }
