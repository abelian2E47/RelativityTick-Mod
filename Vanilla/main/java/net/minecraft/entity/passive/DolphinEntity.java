...
public class DolphinEntity extends WaterAnimalEntity {
    ...
    public void tick() {
        ...
        if (this.isAiDisabled()) {
            ...
            if (this.isWet()) {
                ...
                if (this.isOnGround()) {
                    this.setVelocity(
                        this.getVelocity().add((this.random.nextFloat() * 2.0F - 1.0F) * 0.2F, 0.5, (this.random.nextFloat() * 2.0F - 1.0F) * 0.2F)
                    );
    ...
    static class LeadToNearbyTreasureGoal extends Goal {
        ...
        public void stop() {
            BlockPos blockPos = this.dolphin.getTreasurePos();
            if (BlockPos.ofFloored(blockPos.getX(), this.dolphin.getY(), blockPos.getZ()).isWithinDistance(this.dolphin.getPos(), 4.0)
                || this.noPathToStructure) {
        ...
        public void tick() {
            ...
            if (this.dolphin.isNearTarget() || this.dolphin.getNavigation().isIdle()) {
                ...
                this.dolphin
                    .getLookControl()
                    .lookAt(vec3d2.x, vec3d2.y, vec3d2.z, this.dolphin.getMaxHeadRotation() + 20, this.dolphin.getMaxLookPitchChange());
