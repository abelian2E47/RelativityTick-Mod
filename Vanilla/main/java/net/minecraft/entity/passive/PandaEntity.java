...
public class PandaEntity extends AnimalEntity {
    ...
    private void playEatingAnimation() {
        if (this.getEatingTicks() % 5 == 0) {
            this.playSound(
                SoundEvents.ENTITY_PANDA_EAT, 0.5F + 0.5F * this.random.nextInt(2), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F
            );
            ...
            if ((this.panda.isBaby() || this.panda.isPlayful()) && this.panda.isOnGround()) {
                if (!this.panda.isIdle()) {
                    ...
                    if (this.panda.getWorld().getBlockState(this.panda.getBlockPos().add(i, -1, j)).isAir()) {
                        ...
                        return this.panda.isPlayful() && this.panda.random.nextInt(toGoalTicks(60)) == 1
                            ? true
