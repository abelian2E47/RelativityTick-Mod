...
public class WolfEntity extends TameableEntity implements Angerable, VariantHolder<RegistryEntry<WolfVariant>> {
        ...
        if (this.isTamed()) {
            ...
            if (!(item instanceof DyeItem dyeItem && this.isOwner(player))) {
                ...
                if (!itemStack.isOf(Items.SHEARS)
                    ...
                    || EnchantmentHelper.hasAnyEnchantmentsWith(this.getBodyArmor(), EnchantmentEffectComponentTypes.PREVENT_ARMOR_CHANGE)
                        && !player.isCreative()) {
                    if (this.isInSittingPose()
                        ...
                        ItemStack itemStack3 = this.getBodyArmor();
                        int i = (int)(itemStack3.getMaxDamage() * 0.125F);
                        itemStack3.setDamage(Math.max(0, itemStack3.getDamage() - i));
                        return ActionResult.SUCCESS;
                    }
    ...
    public boolean canAttackWithOwner(LivingEntity target, LivingEntity owner) {
        if (target instanceof CreeperEntity || target instanceof GhastEntity || target instanceof ArmorStandEntity) {
            ...
            && owner instanceof PlayerEntity playerEntity2
    ...
    class AvoidLlamaGoal<T extends LivingEntity> extends FleeEntityGoal<T> {
        ...
        public boolean canStart() {
            return super.canStart() && this.targetEntity instanceof LlamaEntity
                ? !this.wolf.isTamed() && this.isScaredOf((LlamaEntity)this.targetEntity)
                : false;
        }
