...
public class ShulkerBulletEntity extends ProjectileEntity {
    ...
    private void changeTargetDirection(@Nullable Direction.Axis axis) {
        ...
        if (!blockPos.isWithinDistance(this.getPos(), 2.0)) {
            ...
            if (axis != Direction.Axis.X) {
                if (blockPos3.getX() < blockPos.getX() && this.getWorld().isAir(blockPos3.east())) {
                    list.add(Direction.EAST);
                } else if (blockPos3.getX() > blockPos.getX() && this.getWorld().isAir(blockPos3.west())) {
                    list.add(Direction.WEST);
                }
            }

            if (axis != Direction.Axis.Y) {
                if (blockPos3.getY() < blockPos.getY() && this.getWorld().isAir(blockPos3.up())) {
                    list.add(Direction.UP);
                } else if (blockPos3.getY() > blockPos.getY() && this.getWorld().isAir(blockPos3.down())) {
                    list.add(Direction.DOWN);
                }
            }

            if (axis != Direction.Axis.Z) {
                if (blockPos3.getZ() < blockPos.getZ() && this.getWorld().isAir(blockPos3.south())) {
                    list.add(Direction.SOUTH);
                } else if (blockPos3.getZ() > blockPos.getZ() && this.getWorld().isAir(blockPos3.north())) {
            ...
            if (list.isEmpty()) {
                for (int i = 5; !this.getWorld().isAir(blockPos3.offset(direction)) && i > 0; i--) {
                    direction = Direction.random(this.random);
                }
            } else {
    ...
    public void tick() {
        ...
        this.setPosition(this.getPos().add(vec3d2));
        ...
        if (this.getWorld().isClient) {
            this.getWorld().addParticle(ParticleTypes.END_ROD, this.getX() - vec3d2.x, this.getY() - vec3d2.y + 0.15, this.getZ() - vec3d2.z, 0.0, 0.0, 0.0);
