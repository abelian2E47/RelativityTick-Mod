...
public abstract class PersistentProjectileEntity extends ProjectileEntity {
    ...
    public void tick() {
        ...
        if (this.isInGround() && bl) {
            ...
            float h = (float)(MathHelper.atan2(vec3d.y, vec3d.horizontalLength()) * 180.0F / (float)Math.PI);
            this.setPitch(updateRotation(this.getPitch(), h));
    ...
    private void spawnBubbleParticles(Vec3d pos) {
        ...
        for (int i = 0; i < 4; i++) {
            float f = 0.25F;
            this.getWorld()
                .addParticle(ParticleTypes.BUBBLE, pos.x - vec3d.x * 0.25, pos.y - vec3d.y * 0.25, pos.z - vec3d.z * 0.25, vec3d.x, vec3d.y, vec3d.z);
        }
    }
