...
public class ExperimentalMinecartController extends MinecartController {
    ...
    private ExperimentalMinecartController.InterpolatedStep getLerpedStep(float tickDelta) {
        if (tickDelta == this.lastQueriedTickDelta
    ...
    public void adjustToRail(BlockPos pos, BlockState blockState, boolean ignoreWeight) {
        if (AbstractRailBlock.isRail(blockState)) {
            ...
            Vec3d vec3d12 = vec3d10.subtract(vec3d6);
            this.setPos(vec3d6.add(vec3d12));
            float g = 0.0F;
            boolean bl4 = vec3d.getY() != vec3d2.getY();
            if (bl4) {
                Vec3d vec3d13 = pos.toBottomCenterPos().add(vec3d4);
                double d = vec3d13.distanceTo(this.getPos());
                ...
                this.stagingLerpSteps
                    .add(
                        new ExperimentalMinecartController.Step(
                            this.getPos(), this.getVelocity(), this.getYaw(), this.getPitch(), ignoreWeight ? 0.0F : (float)e
                        )
                    );
            }
        }
    ...
    public void moveOnRail(ServerWorld world) {
        for (ExperimentalMinecartController.MoveIteration moveIteration = new ExperimentalMinecartController.MoveIteration();
            ...
            Vec3d vec3d4 = vec3d3.subtract(this.minecart.getLastRenderPos());
            double d = vec3d4.length();
            if (d > 1.0E-5F) {
                if (!(vec3d4.horizontalLengthSquared() > 1.0E-5F)) {
                    if (!this.minecart.isOnRail()) {
                        this.setPitch(this.minecart.isOnGround() ? 0.0F : MathHelper.lerpAngleDegrees(0.2F, this.getPitch(), 0.0F));
                    }
                } else {
                    float f = 180.0F - (float)(Math.atan2(vec3d4.z, vec3d4.x) * 180.0 / Math.PI);
                    float g = this.minecart.isOnGround() && !this.minecart.isOnRail()
                        ? 0.0F
                        : 90.0F - (float)(Math.atan2(vec3d4.horizontalLength(), vec3d4.y) * 180.0 / Math.PI);
                    f += this.minecart.isYawFlipped() ? 180.0F : 0.0F;
    ...
    private Vec3d calcNewHorizontalVelocity(
        ...
        if (iteration.initial) {
            Vec3d vec3d3 = this.applyInitialVelocity(vec3d);
        ...
        if (!iteration.decelerated) {
            Vec3d vec3d4 = this.decelerateFromPoweredRail(vec3d, railState);
            if (vec3d4.horizontalLengthSquared() != vec3d.horizontalLengthSquared()) {
                iteration.decelerated = true;
                vec3d = vec3d4;
            }
        }
        ...
        if (!iteration.accelerated) {
            Vec3d vec3d5 = this.accelerateFromPoweredRail(vec3d, pos, railState);
            if (vec3d5.horizontalLengthSquared() != vec3d.horizontalLengthSquared()) {
