...
public class WitherEntity extends HostileEntity implements RangedAttackMob {
    ...
    public void tickMovement() {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            Entity entity2 = null;
            if (k > 0) {
                entity2 = this.getWorld().getEntityById(k);
            }
        ...
        for (int q = 0; q < 3; q++) {
            ...
            this.getWorld()
                .addParticle(
                    ParticleTypes.SMOKE,
                    r + this.random.nextGaussian() * u,
                    s + this.random.nextGaussian() * u,
                    t + this.random.nextGaussian() * u,
                    0.0,
                    0.0,
                    0.0
                );
            ...
            for (int w = 0; w < 3; w++) {
                this.getWorld()
                    .addParticle(
                        ...
                        this.getY() + this.random.nextFloat() * v,
        ...
        if (this.getInvulnerableTimer() > 0) {
            ...
            for (int j = 1; j < 3; j++) {
                if (this.age >= this.skullCooldowns[j - 1]) {
                    ...
                    if ((world.getDifficulty() == Difficulty.NORMAL || world.getDifficulty() == Difficulty.HARD) && this.chargedSkullCooldowns[j - 1]++ > 15) {
                        float f = 10.0F;
                        float g = 5.0F;
                    ...
                    if (k > 0) {
                        LivingEntity livingEntity = (LivingEntity)world.getEntityById(k);
                        ...
                        List<LivingEntity> list = world.getTargets(
                            LivingEntity.class, HEAD_TARGET_PREDICATE, this, this.getBoundingBox().expand(20.0, 8.0, 20.0)
                        );
                        if (!list.isEmpty()) {
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (entity2 != null && entity2.getType().isIn(EntityTypeTags.WITHER_FRIENDS)) {
            return false;
        }
