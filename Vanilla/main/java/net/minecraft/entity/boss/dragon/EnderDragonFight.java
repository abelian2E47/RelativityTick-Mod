...
public class EnderDragonFight {
    ...
    private BlockPattern.Result findEndPortal() {
        ...
        int k = this.world.getTopPosition(Heightmap.Type.MOTION_BLOCKING, blockPos2).getY();

        for (int l = k; l >= this.world.getBottomY(); l--) {
            BlockPattern.Result result2 = this.endPortalPattern.searchAround(this.world, new BlockPos(blockPos2.getX(), l, blockPos2.getZ()));
            if (result2 != null) {
                if (this.exitPortalLocation == null) {
    ...
    public void dragonKilled(EnderDragonEntity dragon) {
        if (dragon.getUuid().equals(this.dragonUuid)) {
            ...
            if (!this.previouslyKilled) {
                this.world
                    .setBlockState(
                        this.world.getTopPosition(Heightmap.Type.MOTION_BLOCKING, EndPortalFeature.offsetOrigin(this.origin)),
