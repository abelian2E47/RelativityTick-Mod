            ...
            if (this.isAiDisabled()) {
                ...
                if (this.getWorld() instanceof ServerWorld serverWorld2) {
                    ...
                    if (vec3d2 != null) {
                        ...
                        float q = 0.06F;
                        this.updateVelocity(0.06F * (n * p + (1.0F - p)), new Vec3d(0.0, 0.0, -1.0));
                ...
                float t = (float)(this.frameTracker.getFrame(5).y() - this.frameTracker.getFrame(10).y()) * 10.0F * (float) (Math.PI / 180.0);
                ...
                if (this.getWorld() instanceof ServerWorld serverWorld4 && this.hurtTime == 0) {
                    this.launchLivingEntities(
                        serverWorld4,
                        serverWorld4.getOtherEntities(
                    ...
                    this.damageLivingEntities(
                        serverWorld4,
                        serverWorld4.getOtherEntities(this, this.head.getBoundingBox().expand(1.0), EntityPredicates.EXCEPT_CREATIVE_OR_SPECTATOR)
                    );
                    this.damageLivingEntities(
                        serverWorld4,
                        serverWorld4.getOtherEntities(this, this.neck.getBoundingBox().expand(1.0), EntityPredicates.EXCEPT_CREATIVE_OR_SPECTATOR)
                    );
                }
                ...
                float bb = this.getHeadVerticalMovement();
                this.movePart(this.head, z * 6.5F * u, bb + v * 6.5F, -ab * 6.5F * u);
                this.movePart(this.neck, z * 5.5F * u, bb + v * 5.5F, -ab * 5.5F * u);
                ...
                for (int cb = 0; cb < 3; cb++) {
                    ...
                    EnderDragonFrameTracker.Frame frame2 = this.frameTracker.getFrame(12 + cb * 2);
                    float db = this.getYaw() * (float) (Math.PI / 180.0) + this.wrapYawChange(frame2.yRot() - frame.yRot()) * (float) (Math.PI / 180.0);
                    float eb = MathHelper.sin(db);
                    float fb = MathHelper.cos(db);
                    float gb = 1.5F;
                    float hb = (cb + 1) * 2.0F;
                    this.movePart(enderDragonPart, -(x * 1.5F + eb * hb) * u, frame2.y() - frame.y() - (hb + 1.5F) * v + 1.5, (y * 1.5F + fb * hb) * u);
                }

                if (this.getWorld() instanceof ServerWorld serverWorld5) {
                    this.slowedDownByBlock = this.destroyBlocks(serverWorld5, this.head.getBoundingBox())
                        | this.destroyBlocks(serverWorld5, this.neck.getBoundingBox())
                        | this.destroyBlocks(serverWorld5, this.body.getBoundingBox());
                    if (this.fight != null) {
                        this.fight.updateFight(this);
                    }
                }

                for (int ib = 0; ib < this.parts.length; ib++) {
                    this.parts[ib].prevX = vec3ds[ib].x;
                    this.parts[ib].prevY = vec3ds[ib].y;
                    this.parts[ib].prevZ = vec3ds[ib].z;
                    this.parts[ib].lastRenderX = vec3ds[ib].x;
                    this.parts[ib].lastRenderY = vec3ds[ib].y;
                    this.parts[ib].lastRenderZ = vec3ds[ib].z;
                }
            }
        }
    }
    ...
    public int getNearestPathNodeIndex() {
        if (this.pathNodes[0] == null) {
            for (int i = 0; i < 24; i++) {
                ...
                int r = Math.max(73, this.getWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, new BlockPos(l, 0, m)).getY() + j);
