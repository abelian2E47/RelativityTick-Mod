...
public abstract class LivingEntity extends Entity implements Attackable {
    ...
    private static final TrackedData<List<ParticleEffect>> POTION_SWIRLS = DataTracker.registerData(
        LivingEntity.class, TrackedDataHandlerRegistry.PARTICLE_LIST
    );
    ...
    public void baseTick() {
        ...
        if (this.isAlive()) {
            ...
            if (this.isSubmergedIn(FluidTags.WATER)
                ...
                boolean bl2 = !this.canBreatheInWater()
                    && !StatusEffectUtil.hasWaterBreathing(this)
                    && (!bl || !((PlayerEntity)this).getAbilities().invulnerable);
                if (bl2) {
            ...
            if (this.getWorld() instanceof ServerWorld serverWorld3) {
                BlockPos blockPos = this.getBlockPos();
                if (!Objects.equal(this.lastBlockPos, blockPos)) {
                    this.lastBlockPos = blockPos;
                    this.applyMovementEffects(serverWorld3, blockPos);
                }
            }
        }
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (source.getAttacker() instanceof ServerPlayerEntity serverPlayerEntity2) {
            Criteria.PLAYER_HURT_ENTITY.trigger(serverPlayerEntity2, this, source, f, amount, bl);
        }
    ...
    public Map<Enchantment, Set<EnchantmentLocationBasedEffect>> getLocationBasedEnchantmentEffects(EquipmentSlot slot) {
        return this.locationBasedEnchantmentEffects.computeIfAbsent(slot, equipmentSlot -> new Reference2ObjectArrayMap<>());
    }
    ...
    private void onDismounted(Entity vehicle) {
        ...
        if (this.isRemoved()) {
            ...
            if (bl) {
                ...
                Vec3d vec3d3 = vec3d.add(0.0, e, 0.0);
                VoxelShape voxelShape = VoxelShapes.cuboid(Box.of(vec3d3, this.getWidth(), this.getHeight(), this.getWidth()));
                vec3d = this.getWorld()
                    .findClosestCollision(this, voxelShape, vec3d3, this.getWidth(), this.getHeight(), this.getWidth())
    ...
    protected float getJumpVelocity(float strength) {
        return (float)this.getAttributeValue(EntityAttributes.JUMP_STRENGTH) * strength * this.getJumpVelocityMultiplier()
    ...
    private void travelInFluid(Vec3d movementInput) {
        ...
        if (this.horizontalCollision && this.doesNotCollide(vec3d3.x, vec3d3.y + 0.6F - this.getY() + d, vec3d3.z)) {
    ...
    private Vec3d calcGlidingVelocity(Vec3d oldVelocity) {
        ...
        if (f < 0.0F && d > 0.0) {
            double j = e * -MathHelper.sin(f) * 0.04;
            oldVelocity = oldVelocity.add(-vec3d.x * j / d, j * 3.2, -vec3d.z * j / d);
        }
        ...
        if (map != null) {
            for (Entry<EquipmentSlot, ItemStack> entry : map.entrySet()) {
                ...
                if (!itemStack3.isEmpty() && !itemStack3.shouldBreak()) {
                    itemStack3.applyAttributeModifiers(equipmentSlot2, (attribute, modifier) -> {
    ...
    public void tickMovement() {
        ...
        if (this.jumping && this.shouldSwimInFluids()) {
            ...
            double i = this.getSwimHeight();
            if (!bl || this.isOnGround() && !(g > i)) {
                if (!this.isInLava() || this.isOnGround() && !(g > i)) {
                    if ((this.isOnGround() || bl && g <= i) && this.jumpingCooldown == 0) {
                        this.jump();
        ...
        if (!this.getWorld().isClient && !this.isDead()) {
            int j = this.getFrozenTicks();
            if (this.inPowderSnow && this.canFreeze()) {
                this.setFrozenTicks(Math.min(this.getMinFreezeDamageTicks(), j + 1));
            } else {
                this.setFrozenTicks(Math.max(0, j - 2));
            }
        }
        ...
        this.tickCramming();
        profiler.pop();
        if (this.getWorld() instanceof ServerWorld serverWorld2 && this.hurtByWater() && this.isWet()) {
            this.damage(serverWorld2, this.getDamageSources().drown(), 1.0F);
        }
    }
