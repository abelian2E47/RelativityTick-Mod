            ...
            (new MapCodec<Brain<E>>() {
                    ...
                    public <T> Stream<T> keys(DynamicOps<T> ops) {
                        return memoryModules.stream()
                            .flatMap(
                                memoryType -> memoryType.getCodec().map(codec -> Registries.MEMORY_MODULE_TYPE.getId((MemoryModuleType<?>)memoryType)).stream()
                            )
                    ...
                    public <T> DataResult<Brain<E>> decode(DynamicOps<T> ops, MapLike<T> map) {
                        MutableObject<DataResult<Builder<Brain.MemoryEntry<?>>>> mutableObjectx = new MutableObject<>(
                            ...
                            .forEach(
                                pair -> {
                                    ...
                                    DataResult<? extends Brain.MemoryEntry<?>> dataResult2 = dataResult.flatMap(
                                        memoryType -> this.parse((MemoryModuleType<T>)memoryType, ops, (T)pair.getSecond())
                                    );
                    ...
                    private <T, U> DataResult<Brain.MemoryEntry<U>> parse(MemoryModuleType<U> memoryType, DynamicOps<T> ops, T value) {
                        return memoryType.getCodec()
                            ...
                            .<Memory<U>>flatMap(codec -> codec.parse(ops, value))
        ...
        public <T> void serialize(DynamicOps<T> ops, RecordBuilder<T> builder) {
            this.type
                ...
                .ifPresent(
                    codec -> this.data
                        .ifPresent(
                            data -> builder.add(Registries.MEMORY_MODULE_TYPE.getCodec().encodeStart(ops, this.type), codec.encodeStart(ops, (Memory<U>)data))
                        )
                );
        }
    }
