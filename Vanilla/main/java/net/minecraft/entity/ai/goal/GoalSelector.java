    ...
    public void tick() {
        ...
        for (PrioritizedGoal prioritizedGoal2 : this.goals) {
            if (!prioritizedGoal2.isRunning()
                && !usesAny(prioritizedGoal2, this.disabledControls)
                && canReplaceAll(prioritizedGoal2, this.goalsByControl)
                && prioritizedGoal2.canStart()) {
                for (Goal.Control control : prioritizedGoal2.getControls()) {
                    PrioritizedGoal prioritizedGoal3 = this.goalsByControl.getOrDefault(control, REPLACEABLE_GOAL);
                    prioritizedGoal3.stop();
