...
public class MobNavigation extends EntityNavigation {
    ...
    public Path findPathTo(BlockPos target, int distance) {
        ...
        BlockPos.Mutable mutable2 = target.mutableCopy().move(Direction.UP);

        while (mutable2.getY() <= this.world.getTopYInclusive() && worldChunk.getBlockState(mutable2).isSolid()) {
            mutable2.move(Direction.UP);
        }

        return super.findPathTo(mutable2.toImmutable(), distance);
    }
