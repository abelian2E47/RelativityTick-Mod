...
public class ExperienceCommand {
                                    ...
                                    CommandManager.literal("points")
                                        .executes(
                                            context -> executeQuery(
                                                context.getSource(), EntityArgumentType.getPlayer(context, "target"), ExperienceCommand.Component.POINTS
                                            )
                                        )
                                )
                                .then(
                                    CommandManager.literal("levels")
                                        .executes(
                                            context -> executeQuery(
                                                context.getSource(), EntityArgumentType.getPlayer(context, "target"), ExperienceCommand.Component.LEVELS
                                            )
                                        )
                                )
                        )
    ...
    private static int executeAdd(
        ...
        if (targets.size() == 1) {
            source.sendFeedback(
                () -> Text.translatable("commands.experience.add." + component.name + ".success.single", amount, targets.iterator().next().getDisplayName()),
                true
            );
        } else {
