                            ...
                            CommandManager.argument("testName", TestFunctionArgumentType.testFunction())
                                ...
                                .then(
                                    CommandManager.argument("amount", IntegerArgumentType.integer())
                                        .executes(
                                            context -> RUNNERS.repeat(IntegerArgumentType.getInteger(context, "amount"))
                            ...
                            CommandManager.argument("testName", TestFunctionArgumentType.testFunction())
                                .executes(
                                    context -> RUNNERS.structureNamed(
                                            context, "minecraft:" + TestFunctionArgumentType.getFunction(context, "testName").templateName()
                                        )
                                        .locate()
                                )
                        )
                    ...
                    CommandManager.literal("clearall")
                        ...
                        .then(
                            CommandManager.argument("radius", IntegerArgumentType.integer())
                                .executes(
                                    context -> RUNNERS.surface(context, MathHelper.clamp(IntegerArgumentType.getInteger(context, "radius"), 0, 1024)).clear()
                                )
                        )
                )
                .then(
        ...
        int verify() {
            ...
            for (GameTestState gameTestState : collection) {
                for (BlockRotation blockRotation : BlockRotation.values()) {
                    ...
                    for (int i = 0; i < 100; i++) {
                        GameTestState gameTestState2 = new GameTestState(
                            gameTestState.getTestFunction(), blockRotation, serverWorld, new TestAttemptConfig(1, true)
                        );
