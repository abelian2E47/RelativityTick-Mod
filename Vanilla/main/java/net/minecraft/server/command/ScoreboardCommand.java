...
public class ScoreboardCommand {
                                    ...
                                    CommandManager.argument("objective", ScoreboardObjectiveArgumentType.scoreboardObjective())
                                        .executes(
                                            context -> executeRemoveObjective(
                                                context.getSource(), ScoreboardObjectiveArgumentType.getObjective(context, "objective")
                                            )
                                        )
                                )
                        )
                            ...
                            CommandManager.literal("setdisplay")
                                .then(
                                    CommandManager.argument("slot", ScoreboardSlotArgumentType.scoreboardSlot())
                                        .executes(
                                            context -> executeClearDisplay(context.getSource(), ScoreboardSlotArgumentType.getScoreboardSlot(context, "slot"))
                                        )
                                        .then(
                                            CommandManager.argument("objective", ScoreboardObjectiveArgumentType.scoreboardObjective())
                                                .executes(
                            ...
                            CommandManager.literal("reset")
                                .then(
                                    CommandManager.argument("targets", ScoreHolderArgumentType.scoreHolders())
                                        .suggests(ScoreHolderArgumentType.SUGGESTION_PROVIDER)
                                        .executes(
                                            context -> executeReset(context.getSource(), ScoreHolderArgumentType.getScoreboardScoreHolders(context, "targets"))
                                        )
                                        .then(
                                            CommandManager.argument("objective", ScoreboardObjectiveArgumentType.scoreboardObjective())
                                                .executes(
                                            ...
                                            CommandManager.argument("objective", ScoreboardObjectiveArgumentType.scoreboardObjective())
                                                .suggests(
                                                    (context, builder) -> suggestDisabled(
                                                        context.getSource(), ScoreHolderArgumentType.getScoreboardScoreHolders(context, "targets"), builder
                                                    )
                                                )
                                                .executes(
        ...
        for (ScoreboardCriterion.RenderType renderType : ScoreboardCriterion.RenderType.values()) {
            literalArgumentBuilder.then(
                CommandManager.literal(renderType.getName())
                    .executes(
                        context -> executeModifyRenderType(context.getSource(), ScoreboardObjectiveArgumentType.getObjective(context, "objective"), renderType)
                    )
            );
        }
    ...
    private static int executeOperation(
        ...
        if (targets.size() == 1) {
            ...
            source.sendFeedback(
                () -> Text.translatable(
                    "commands.scoreboard.players.operation.success.single", targetObjective.toHoverableText(), getNextDisplayName(targets), j
                ),
                true
            );
        } else {
    ...
    private static int executeReset(ServerCommandSource source, Collection<ScoreHolder> targets, ScoreboardObjective objective) {
        ...
        if (targets.size() == 1) {
            ...
            source.sendFeedback(
                () -> Text.translatable("commands.scoreboard.players.reset.specific.multiple", objective.toHoverableText(), targets.size()), true
            );
        }
    ...
    private static int executeSet(ServerCommandSource source, Collection<ScoreHolder> targets, ScoreboardObjective objective, int score) {
        ...
        if (targets.size() == 1) {
            source.sendFeedback(
                () -> Text.translatable("commands.scoreboard.players.set.success.single", objective.toHoverableText(), getNextDisplayName(targets), score),
                true
            );
        } else {
            source.sendFeedback(
    ...
    private static int executeSetDisplayName(
        ...
        if (displayName == null) {
            if (targets.size() == 1) {
                ...
                source.sendFeedback(
                    () -> Text.translatable("commands.scoreboard.players.display.name.clear.success.multiple", targets.size(), objective.toHoverableText()),
                    true
                );
            }
            ...
            source.sendFeedback(
                () -> Text.translatable(
                    "commands.scoreboard.players.display.name.set.success.multiple", displayName, targets.size(), objective.toHoverableText()
                ),
                true
            );
        }
    ...
    private static int executeSetNumberFormat(
        ...
        if (numberFormat == null) {
            if (targets.size() == 1) {
                ...
                source.sendFeedback(
                    () -> Text.translatable(
                        "commands.scoreboard.players.display.numberFormat.clear.success.multiple", targets.size(), objective.toHoverableText()
                    ),
                    true
                );
            }
            ...
            source.sendFeedback(
                () -> Text.translatable("commands.scoreboard.players.display.numberFormat.set.success.multiple", targets.size(), objective.toHoverableText()),
                true
            );
        }
    ...
    private static int executeAdd(ServerCommandSource source, Collection<ScoreHolder> targets, ScoreboardObjective objective, int score) {
        ...
        if (targets.size() == 1) {
            int j = i;
            source.sendFeedback(
                () -> Text.translatable("commands.scoreboard.players.add.success.single", score, objective.toHoverableText(), getNextDisplayName(targets), j),
                true
            );
        } else {
            source.sendFeedback(
    ...
    private static int executeListPlayers(ServerCommandSource source) {
        ...
        if (collection.isEmpty()) {
            ...
            source.sendFeedback(
                () -> Text.translatable(
                    "commands.scoreboard.players.list.success", collection.size(), Texts.join(collection, ScoreHolder::getStyledDisplayName)
                ),
                false
            );
        }
