    ...
    private static int executeSpawnLeader(ServerCommandSource source) {
        ...
        if (raiderEntity == null) {
            ...
            raiderEntity.initialize(
                source.getWorld(), source.getWorld().getLocalDifficulty(BlockPos.ofFloored(source.getPosition())), SpawnReason.COMMAND, null
            );
    ...
    private static int executeSound(ServerCommandSource source, @Nullable Text type) {
        if (type != null && type.getString().equals("local")) {
            ...
            serverWorld.playSound(
                null, vec3d.x, vec3d.y, vec3d.z, SoundEvents.EVENT_RAID_HORN, SoundCategory.NEUTRAL, 2.0F, 1.0F, serverWorld.random.nextLong()
            );
        }
