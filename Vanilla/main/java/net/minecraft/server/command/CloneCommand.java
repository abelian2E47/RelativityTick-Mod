                    ...
                    CommandManager.literal("from")
                        .then(
                            CommandManager.argument("sourceDimension", DimensionArgumentType.dimension())
                                .then(
                                    createSourceArgs(commandRegistryAccess, context -> DimensionArgumentType.getDimensionArgument(context, "sourceDimension"))
                                )
                        )
                )
        );
                                ...
                                CommandManager.argument("targetDimension", DimensionArgumentType.dimension())
                                    .then(
                                        createDestinationArgs(
                                            ...
                                            worldGetter,
                                            context -> DimensionArgumentType.getDimensionArgument(context, "targetDimension")
                                        )
                                    )
                            )
                    )
        ...
        if (serverWorld.isRegionLoaded(blockPos, blockPos2) && serverWorld2.isRegionLoaded(blockPos3, blockPos4)) {
            ...
            if (mode == CloneCommand.Mode.MOVE) {
                ...
                for (BlockPos blockPos9 : deque) {
                    serverWorld.setBlockState(blockPos9, Blocks.AIR.getDefaultState(), Block.NOTIFY_ALL);
                }
            }
            ...
            for (CloneCommand.BlockInfo blockInfo3 : list2) {
                BlockEntity blockEntity4 = serverWorld2.getBlockEntity(blockInfo3.pos);
                if (blockInfo3.blockEntityInfo != null && blockEntity4 != null) {
                    blockEntity4.readComponentlessNbt(blockInfo3.blockEntityInfo.nbt, serverWorld2.getRegistryManager());
                    blockEntity4.setComponents(blockInfo3.blockEntityInfo.components);
                    blockEntity4.markDirty();
                }

                serverWorld2.setBlockState(blockInfo3.pos, blockInfo3.state, Block.NOTIFY_LISTENERS);
            }

            for (CloneCommand.BlockInfo blockInfo4 : list5) {
                serverWorld2.updateNeighbors(blockInfo4.pos, blockInfo4.state.getBlock());
            }
