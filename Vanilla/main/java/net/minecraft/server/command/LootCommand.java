...
public class LootCommand {
                    ...
                    .then(
                        sourceConstructor.construct(
                            ...
                            (context, stacks, messageSender) -> executeSpawn(
                                context.getSource(), Vec3ArgumentType.getVec3(context, "targetPos"), stacks, messageSender
                            )
                        )
                    )
            );
    ...
    private static void sendDroppedFeedback(ServerCommandSource source, List<ItemStack> stacks, RegistryKey<LootTable> lootTable) {
        if (stacks.size() == 1) {
            ...
            source.sendFeedback(
                () -> Text.translatable(
                    "commands.drop.success.single_with_table", itemStack.getCount(), itemStack.toHoverableText(), Text.of(lootTable.getValue())
                ),
                false
            );
        } else {
