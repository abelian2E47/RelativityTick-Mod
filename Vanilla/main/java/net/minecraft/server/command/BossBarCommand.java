                                    ...
                                    CommandManager.literal("name")
                                        .then(
                                            CommandManager.argument("name", TextArgumentType.text(registryAccess))
                                                .executes(
                                                    context -> setName(
                                                        context.getSource(), getBossBar(context), TextArgumentType.getTextArgument(context, "name")
                                                    )
                                                )
                                        )
                                )
                                    ...
                                    CommandManager.literal("color")
                                        .then(
                                            CommandManager.literal("pink")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.PINK))
                                        )
                                        .then(
                                            CommandManager.literal("blue")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.BLUE))
                                        )
                                        .then(
                                            CommandManager.literal("red")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.RED))
                                        )
                                        .then(
                                            CommandManager.literal("green")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.GREEN))
                                        )
                                        .then(
                                            CommandManager.literal("yellow")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.YELLOW))
                                        )
                                        .then(
                                            CommandManager.literal("purple")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.PURPLE))
                                        )
                                        .then(
                                            CommandManager.literal("white")
                                                .executes(context -> setColor(context.getSource(), getBossBar(context), BossBar.Color.WHITE))
                                        )
                                )
                                .then(
                                    CommandManager.literal("style")
                                        .then(
                                            CommandManager.literal("progress")
                                                .executes(context -> setStyle(context.getSource(), getBossBar(context), BossBar.Style.PROGRESS))
                                        )
                                        .then(
                                            CommandManager.literal("notched_6")
                                                .executes(context -> setStyle(context.getSource(), getBossBar(context), BossBar.Style.NOTCHED_6))
                                        )
                                        .then(
                                            CommandManager.literal("notched_10")
                                                .executes(context -> setStyle(context.getSource(), getBossBar(context), BossBar.Style.NOTCHED_10))
                                        )
                                        .then(
                                            CommandManager.literal("notched_12")
                                                .executes(context -> setStyle(context.getSource(), getBossBar(context), BossBar.Style.NOTCHED_12))
                                        )
                                        .then(
                                            CommandManager.literal("notched_20")
                                                .executes(context -> setStyle(context.getSource(), getBossBar(context), BossBar.Style.NOTCHED_20))
                                        )
                                )
                                .then(
                                    ...
                                    CommandManager.literal("max")
                                        .then(
                                            CommandManager.argument("max", IntegerArgumentType.integer(1))
                                                .executes(
                                                    context -> setMaxValue(
                                                        context.getSource(), getBossBar(context), IntegerArgumentType.getInteger(context, "max")
                                                    )
                                                )
                                        )
                                )
                                    ...
                                    CommandManager.literal("players")
                                        ...
                                        .then(
                                            CommandManager.argument("targets", EntityArgumentType.players())
                                                .executes(
                                                    context -> setPlayers(
                                                        context.getSource(), getBossBar(context), EntityArgumentType.getOptionalPlayers(context, "targets")
                                                    )
                                                )
                                        )
                                )
