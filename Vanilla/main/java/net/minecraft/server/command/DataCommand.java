                            ...
                            CommandManager.argument("nbt", NbtCompoundArgumentType.nbtCompound())
                                .executes(
                                    context -> executeMerge(
                                        context.getSource(), objectType.getObject(context), NbtCompoundArgumentType.getNbtCompound(context, "nbt")
                                    )
                                )
                        )
                    )
                        ...
                        builder -> builder.executes(context -> executeGet(context.getSource(), objectType.getObject(context)))
                            .then(
                                CommandManager.argument("path", NbtPathArgumentType.nbtPath())
                                    .executes(
                                        context -> executeGet(
                                            context.getSource(), objectType.getObject(context), NbtPathArgumentType.getNbtPath(context, "path")
                                        )
                                    )
                                    .then(
                        ...
                        (builder, modifier) -> builder.then(
                            ...
                            .then(
                                CommandManager.literal("set")
                                    .then(modifier.create((context, sourceNbt, path, elements) -> path.put(sourceNbt, Iterables.getLast(elements))))
                            )
                            .then(CommandManager.literal("merge").then(modifier.create((context, element, path, elements) -> {
                                ...
                                for (NbtElement nbtElement2 : collection) {
                                    ...
                                    NbtCompound nbtCompound5 = nbtCompound3.copy();
                                    nbtCompound3.copyFrom(nbtCompound);
                                    i += nbtCompound5.equals(nbtCompound3) ? 0 : 1;
                                }
        ...
        for (DataCommand.ObjectType objectType : TARGET_OBJECT_TYPES) {
            objectType.addArgumentsToBuilder(
                ...
                builder -> {
                    ...
                    subArgumentAdder.accept(
                        argumentBuilder,
                        modifier -> CommandManager.literal("value")
