...
public class TeamCommand {
                                    ...
                                    CommandManager.literal("collisionRule")
                                            ...
                                            CommandManager.literal("never")
                                                .executes(
                                                    context -> executeModifyCollisionRule(
                                                        context.getSource(), TeamArgumentType.getTeam(context, "team"), AbstractTeam.CollisionRule.NEVER
                                                    )
                                                )
                                        )
                                        .then(
                                        ...
                                        .then(
                                            CommandManager.literal("always")
                                                .executes(
                                                    context -> executeModifyCollisionRule(
                                                        context.getSource(), TeamArgumentType.getTeam(context, "team"), AbstractTeam.CollisionRule.ALWAYS
                                                    )
                                                )
                                        )
                                )
    ...
    private static int executeModifyFriendlyFire(ServerCommandSource source, Team team, boolean allowed) throws CommandSyntaxException {
        if (team.isFriendlyFireAllowed() == allowed) {
            ...
            source.sendFeedback(
                () -> Text.translatable("commands.team.option.friendlyfire." + (allowed ? "enabled" : "disabled"), team.getFormattedName()), true
            );
            return 0;
        }
    }
