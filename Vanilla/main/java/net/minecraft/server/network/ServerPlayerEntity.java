...
public class ServerPlayerEntity extends PlayerEntity {
    ...
    public void readRootVehicle(Optional<NbtCompound> nbt) {
        if (nbt.isPresent() && nbt.get().contains("RootVehicle", NbtElement.COMPOUND_TYPE) && this.getWorld() instanceof ServerWorld serverWorld) {
    ...
    public void readEnderPearls(Optional<NbtCompound> nbt) {
        if (nbt.isPresent() && nbt.get().contains("ender_pearls", NbtElement.LIST_TYPE) && nbt.get().get("ender_pearls") instanceof NbtList nbtList) {
            nbtList.forEach(
                enderPearlNbt -> {
    ...
    protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
        if (this.spawnExtraParticlesOnFall && onGround && this.fallDistance > 0.0F) {
            ...
            this.getServerWorld()
                .spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, state), vec3d.x, vec3d.y, vec3d.z, i, 0.3F, 0.3F, 0.3F, 0.15F);
            ...
            if (this.isSwimming()) {
                ...
                if (l > 0) {
                    if (this.isSprinting()) {
                        ...
                        this.addExhaustion(0.1F * l * 0.01F);
                    } else if (this.isInSneakingPose()) {
                        this.increaseStat(Stats.CROUCH_ONE_CM, l);
                        this.addExhaustion(0.0F * l * 0.01F);
                    } else {
    ...
    private ItemEntity dropPlayerItem(ItemStack stack, boolean throwRandomly, boolean retainOwnership) {
        ...
        if (throwRandomly) {
            ...
            itemEntity.setVelocity(
                -k * j * 0.3F + Math.cos(m) * n, -i * 0.3F + 0.1F + (this.random.nextFloat() - this.random.nextFloat()) * 0.1F, l * j * 0.3F + Math.sin(m) * n
            );
        }
