...
public class ServerChunkLoadingManager extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider, ChunkLoadingManager {
    ...
    public ServerChunkLoadingManager(
        ...
        if (chunkGenerator instanceof NoiseChunkGenerator noiseChunkGenerator) {
            this.noiseConfig = NoiseConfig.create(
                noiseChunkGenerator.getSettings().value(), dynamicRegistryManager.getOrThrow(RegistryKeys.NOISE_PARAMETERS), l
            );
        } else {
            this.noiseConfig = NoiseConfig.create(
    ...
    protected IntSupplier getCompletedLevelSupplier(long pos) {
        return () -> {
            ...
            return chunkHolder == null
                ...
                : Math.min(chunkHolder.getCompletedLevel(), LevelPrioritizedQueue.LEVEL_COUNT - 1);
        };
    }
        ...
        Consumer<ChunkHolder> consumer = chunkHolder -> chunkHolder.enumerateFutures()
            .forEach(
                pair -> {
                    ...
                    CompletableFuture<OptionalChunk<Chunk>> completableFuture = pair.getSecond();
        ...
        completableFuture.thenRunAsync(() -> {
            ...
            if (completableFuture2 != completableFuture) {
                ...
                if (this.chunksToUnload.remove(pos, chunk) && chunkx != null) {
                    ...
                    this.save(chunkx);
                    if (chunkx instanceof WorldChunk worldChunk2) {
                        this.world.unloadEntities(worldChunk2);
                    }
    ...
    private void sendWatchPackets(ServerPlayerEntity player) {
        ...
        if (!(
            player.getChunkFilter() instanceof ChunkFilter.Cylindrical cylindrical && cylindrical.center().equals(chunkPos) && cylindrical.viewDistance() == i
        )) {
    ...
    class EntityTracker {
        ...
        public EntityTracker(final Entity entity, final int maxDistance, final int tickInterval, final boolean alwaysUpdateVelocity) {
            this.entry = new EntityTrackerEntry(
                ServerChunkLoadingManager.this.world, entity, tickInterval, alwaysUpdateVelocity, this::sendToOtherNearbyPlayers
            );
            this.entity = entity;
