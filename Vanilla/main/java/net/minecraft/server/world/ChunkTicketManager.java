...
public abstract class ChunkTicketManager {
    ...
    public boolean update(ServerChunkLoadingManager chunkLoadingManager) {
        ...
        if (!this.chunkHoldersWithPendingUpdates.isEmpty()) {
            ...
            for (ChunkHolder chunkHolder2 : this.chunkHoldersWithPendingUpdates) {
                chunkHolder2.updateFutures(chunkLoadingManager, this.mainThreadExecutor);
            }
            ...
            if (!this.freshPlayerTicketPositions.isEmpty()) {
                ...
                while (longIterator.hasNext()) {
                    ...
                    if (this.getTicketSet(l).stream().anyMatch(ticket -> ticket.getType() == ChunkTicketType.PLAYER)) {
                        ChunkHolder chunkHolder3 = chunkLoadingManager.getCurrentChunkHolder(l);
                        if (chunkHolder3 == null) {
                            throw new IllegalStateException();
                        }

                        CompletableFuture<OptionalChunk<WorldChunk>> completableFuture = chunkHolder3.getEntityTickingFuture();
            ...
            if (oldWithinViewDistance != withinViewDistance) {
                ...
                if (withinViewDistance) {
                    ...
                    ChunkTicketManager.this.scheduler
                        .remove(
                            pos, () -> ChunkTicketManager.this.mainThreadExecutor.execute(() -> ChunkTicketManager.this.removeTicket(pos, chunkTicket)), true
                        );
                }
            }
        }
