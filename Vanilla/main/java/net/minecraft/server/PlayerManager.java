...
public abstract class PlayerManager {
    ...
    public void onPlayerConnect(ClientConnection connection, ServerPlayerEntity player, ConnectedClientData clientData) {
        ...
        Optional<NbtCompound> optional2 = this.loadPlayerData(player);
        RegistryKey<World> registryKey = optional2.<RegistryKey<World>>flatMap(
        ...
        String string3 = connection.getAddressAsString(this.server.shouldLogIps());
        LOGGER.info(
        ...
        player.readGameModeNbt(optional2.orElse(null));
        ...
        player.readEnderPearls(optional2);
        player.readRootVehicle(optional2);
        player.onSpawn();
    }
    ...
    public Text checkCanJoin(SocketAddress address, GameProfile profile) {
        if (this.bannedProfiles.contains(profile)) {
            ...
            if (this.bannedIps.isBanned(address)) {
                ...
                MutableText mutableText2 = Text.translatable("multiplayer.disconnect.banned_ip.reason", bannedIpEntry.getReason());
                if (bannedIpEntry.getExpiryDate() != null) {
                    mutableText2.append(Text.translatable("multiplayer.disconnect.banned_ip.expiration", DATE_FORMATTER.format(bannedIpEntry.getExpiryDate())));
                }
    ...
    public void sendWorldInfo(ServerPlayerEntity player, ServerWorld world) {
        ...
        player.networkHandler
            .sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.INITIAL_CHUNKS_COMING, GameStateChangeS2CPacket.DEMO_OPEN_SCREEN));
